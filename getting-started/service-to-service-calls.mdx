...
**Note:** The [cpln apply](/guides/cpln-apply) command is used to create the Workloads that are defined in the YAML manifest file. The file can be viewed by clicking [here](https://raw.githubusercontent.com/controlplane-com/examples/main/quickstarts/5/01_create_workloads.yaml).
...
<Tip>
  If you received the same response, this demonstrated that the `client` Workload was able to call the `server`
  Workload internally.
</Tip>
