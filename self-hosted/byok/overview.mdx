...
3. Enter any optional [tags](/common-configs/tags). Click `Create`.
...
- [Agent Configuration:](/self-hosted/byok/settings/agent) Agent settings are used during the installation and upgrade of all BYOK components.
- [Actuator Settings:](/self-hosted/byok/settings/actuator) Actuator settings are used to control ingress and other changes to the way workloads are created in Kubernetes.
- [Workload Settings:](/self-hosted/byok/settings/workload) Additional workload settings when using a BYOK Location.
- [Volume Set Settings:](/self-hosted/byok/settings/volumeset) Additional volume set settings when using a BYOK Location.

## Guides

- [Configure a Content Delivery Network (CDN) Domain.](/guides/configure-resources/configure-cdn#amazon-cloudfront-configuration-steps)
