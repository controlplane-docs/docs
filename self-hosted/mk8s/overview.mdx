---
title: Managed Kubernetes
---

The Managed Kubernetes service developed by Control Plane is designed to facilitate the creation and management (i.e., upgrades and scaling) of Kubernetes clusters across multiple cloud platforms and private cloud environments. By offering a unified view for Kubernetes cluster management, this service enables the centralized management of numerous clusters on various cloud platforms, private clouds, and across accounts.

Managed Kubernetes handles the tasks required to maintain a healthy Kubernetes cluster. In addition, it manages autoscaling according to user preferences, optimizes costs by using spot instances when provided, and allows seamless upgrades based on the user's selected version.

## Supported Kubernetes versions

- 1.28.4 (latest)
- 1.28.2
- 1.27.3
- 1.26.4
- 1.26.0

## Infrastructure Providers

Click on provider name for details.

1. [Generic](/self-hosted/mk8s/generic)
2. [AWS](/self-hosted/mk8s/aws)
3. [Hetzner](/self-hosted/mk8s/hetzner)

## Add-Ons

Enables the user to turn on or off the following optional features.

- [Dashboard:](/self-hosted/mk8s/add-ons/dashboard) Provides a Kubernetes dashboard UI for the cluster.
- [AWS Workload Identity:](/self-hosted/mk8s/add-ons/aws_workload_identity) Allows your pods to assume AWS IAM Roles.
- [AWS ECR:](/self-hosted/mk8s/add-ons/aws_ecr) Allows pulling images from private ECR registries.
- [AWS EFS:](/self-hosted/mk8s/add-ons/aws_efs) Provides support for persistent volumes using AWS Elastic File System.
- [Local Path Storage:](/self-hosted/mk8s/add-ons/local_path_storage) Create PVCs backed by local volumes.
- [Logs:](/self-hosted/mk8s/add-ons/logs) Enable logging for pods and cluster auditing.

## Permissions

The permissions below are used to define [policies](/objects/policy) together with one or more of the four
[principal types](/platform/access-control):

| Permission   | Description                | Implies                                          |
| :----------- | :------------------------- | :----------------------------------------------- |
| clusterAdmin | Full access to the K8S API |                                                  |
| create       | Create new clusters        |                                                  |
| delete       | Delete a cluster           |                                                  |
| edit         | Modify existing clusters   | view                                             |
| manage       | Full access                | clusterAdmin, create, delete, edit, manage, view |
| view         | Read-only view             |                                                  |
