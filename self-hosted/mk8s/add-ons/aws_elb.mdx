...
The [AWS Workload Identity](/self-hosted/mk8s/add-ons/aws_workload_identity) Add-On is leveraged in the configuraiton of the load balancer controller to configure access for this role.
...
If the controller cannot access the AWS API, verify that you have performed [Step 1 for the AWS Identity Add-On](/self-hosted/mk8s/add-ons/aws_workload_identity#steps-to-configure-access).
