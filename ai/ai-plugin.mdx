...
<Note>
  The plugin bundles the MCP server and auto-selects the [toolset profile](/ai/mcp-server#toolset-profiles) per client — Claude Code gets `full`, while Codex and Gemini CLI get `core`.
</Note>
