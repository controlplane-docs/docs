...
<Steps>
  <Step title="Register the server">
    ...
    <Note>
      `?toolsets=full` serves the complete tool catalog. Claude Code loads tool schemas on demand, so the full catalog costs nothing up front — other clients should keep the default profile instead (see [toolset profiles](/ai/mcp#toolset-profiles)).
    </Note>
