...
keywords: ['mcp tools', 'tool list', 'tool reference', 'toolsets', 'readonly profile', 'core profile', 'mk8s profile', 'full profile', 'read only', 'destructive', 'cpln mcp']
---
...
| `core` | 55 | `https://mcp.cpln.io/mcp` | The default. Everyday workflows — workloads, GVCs, image builds, domains, identities, policies, templates, volume sets, observability (logs, metrics, traces, audit), quotas, and Terraform export. |
| `mk8s` | 77 | `https://mcp.cpln.io/mcp?toolsets=mk8s` | Everything in `core` plus the [managed Kubernetes](/mk8s/overview) family for provisioning and updating clusters across cloud providers. |
| `full` | 131 | `https://mcp.cpln.io/mcp?toolsets=full` | Every tool, including agents, cloud accounts, IP sets, service accounts, audit contexts, external logging, quotas, advanced workload configuration, and volume snapshots. |
...
Tools the assistant uses to ground itself in accurate Control Plane facts, schemas, and documentation before acting. The three documentation tools (`search_control_plane`, `query_docs_filesystem_control_plane`, `submit_feedback`) are proxied from the standalone docs MCP at `docs.controlplane.com/mcp`; the rest are served by the MCP Server itself.
...
| `submit_feedback` | Submit Feedback | Report a page in this documentation that is wrong, outdated, or incomplete, so the docs team can fix it. | Write · `full` |
...
| `workload_stop_replica` | Workload — Stop a Replica | Terminate one running replica of a workload; the platform reschedules a replacement per the scaling settings. | Destructive |
...
| `add_ipset_location` | Add or update IP set locations | Add locations to an IP set or update their retention policy. Passing an existing location overwrites its entry, and retention policy `free` releases an allocated static IP. | Destructive · `full` |
...
| `edit_audit_context` | Edit an Audit Context | Update an audit context's description and tags. Replaces the description and can clear existing tags. | Destructive · `full` |
...
<CardGroup cols={2}>
  ...
  <Card title="Reference" icon="book-open" href="/reference/overview">
