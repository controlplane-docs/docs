...
keywords: ['east-west', 'internal traffic', 'cpln.local', 'private endpoint', 'inter-workload', 'service mesh', 'zero trust', 'tutorial', 'walkthrough', 'workload to workload']
---
...
- Deploy a new workload that communicates an existing service
...
All internal traffic is automatically encrypted with mTLS. No certificate management required.
...
The `hello-world` workload redeploys with the updated firewall rules (1-2 minutes).
</Step>
...
- **mTLS is automatic** - no certificate configuration required
