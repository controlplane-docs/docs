...
Connect two workloads over Control Plane's internal network: deploy a caller workload, watch the default deny block its request, then allow same-GVC traffic and verify the call succeeds. Internal traffic is encrypted with mTLS automatically; no certificate management is required.
...
The `hello-world` workload redeploys with the new rules. Once it reports `Ready` again, refresh the caller page with the `?url=` parameter; the response now comes from `hello-world`:
...
To stop here instead, delete all resources created through the quickstart series:
...
<Card title="5. Add a database and wire secrets" icon="database" href="/quickstart/quick-start-5-database-secrets" horizontal>
  Install a PostgreSQL template and connect a workload to it through the secret access chain.
</Card>
