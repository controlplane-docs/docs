...
Replace `example.com` with your domain, or leave the domain out if you did not create one. The agent lists what goes: the domain, the GVC with `web`, `frontend`, and `api`, and the images. Confirm, and it deletes them.
...
<Note>
Remove the DNS records from your DNS provider after deleting the domain. A `--remote` build also pushes the build cache images `frontend-cache:latest` and `api-cache:latest`; delete those too if you built without Docker.
</Note>
