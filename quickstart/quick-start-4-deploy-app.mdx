...
The Control Plane [CLI](/cli-reference/overview) offers developers the ability to containerize their code and push the resulting [image](/reference/image) to their [Org's](/reference/org) private [image registry](/reference/image). The [image registry](/reference/image) is scoped to the [Org](/reference/org) and available for use by all [Workloads](/reference/workload) within the [Org](/reference/org). Along with the console, the [CLI](/cli-reference/overview) can be used to deploy the [image](/reference/image) to the cloud by configuring a [GVC](/reference/gvc) and [Workload](/reference/workload).

This quick start will demonstrate how to:

- Containerize a Node.js application.
- Configure a [GVC](/reference/gvc) and [Workload](/reference/workload) using the [CLI](/cli-reference/overview).
...
<Note>
  ...
  [profile](/cli-reference/get-started/profiles), you can omit the `--org` flag.
</Note>
...
Review the [Push an Image](/cli-reference/guides/push-image) guide for additional details.

## Step Two - Configure a GVC

In this step, you will configure a new [GVC](/reference/gvc) for the application using the [CLI](/cli-reference/overview). If you'd like to use the [GVC](/reference/gvc) created in the previous [quick start](/quickstart/sample-application), skip to [step 3](#step-three-configure-a-workload).
...
In the previous [quick start](/quickstart/sample-application), you used the console to deploy an application, here you exercised the [CLI](/cli-reference/overview) to perform similar actions. All Control Plane resources, such as [GVCs](/reference/gvc) and [Workloads](/reference/workload), can be created/modified either using the console or the [CLI](/cli-reference/overview).
