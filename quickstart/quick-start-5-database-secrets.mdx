...
Once `db-postgres` and the new `api` version report `Ready`, reload the frontend. The badge next to the `Recent signups` heading now says `Stored in PostgreSQL` with the location nearest to you, and the list is empty because the part 4 signups lived in memory. Join the waitlist, then open another location's endpoint from the `Deployments` page. It shows the same list, because every API replica now writes to the one database in `quickstart-db`.
