...
Open each location's endpoint from the `Deployments` page (or the `cpln workload get-deployments frontend --gvc quickstart-gvc` output) and the footer switches to the other location.
...
- **Image references**: the `//image/` prefix for the CLI and the Console, the full `/org/<org>/image/` path for Terraform and Pulumi.
