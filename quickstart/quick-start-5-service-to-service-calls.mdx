...
- [Permissions](/reference/policy#permissions) to:
  ...
  - Create and edit a [Workload](/reference/workload).

- Latest [CLI](/cli-reference/installation) installed.
...
1. Authenticate to Control Plane by executing the following [CLI](/cli-reference/overview) command:

```
cpln login
```

2. Execute the following [CLI](/cli-reference/overview) command to update your profile with a default [Org](/reference/org) which will be used by the subsequent commands (substitute ORG_NAME for your org):

```
cpln profile update default --org ORG_NAME
```

3. Execute the following [CLI](/cli-reference/overview) command to create a [GVC](/reference/gvc) that will contain the two [Workloads](/reference/workload):
...
4. Execute the following [CLI](/cli-reference/overview) command to create the two [Workloads](/reference/workload) that will be hosting the sample application.

**Note:** The [cpln apply](/cli-reference/guides/cpln-apply) command is used to create the [Workloads](/reference/workload) that are defined in the YAML manifest file. The file can be viewed by clicking [here](https://raw.githubusercontent.com/controlplane-com/examples/main/quickstarts/5/01_create_workloads.yaml).
...
The first [Workload](/reference/workload) is named `server` and the second is named `client`. They are both configured with the sample application image and the firewall rules have been set to allow outside traffic.
...
Execute the following [CLI](/cli-reference/overview) command to update the firewall rules of the `server` [Workload](/reference/workload) to grant inbound access from any [Workload](/reference/workload) in the same [GVC](/reference/gvc):
