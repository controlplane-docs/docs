...
Replace `example.com` with your domain, or leave the domain out if you did not create one. The agent lists what goes, including the release's volume set and the data on it and the tracing setting with its GVC, and asks you to confirm. The agent cannot delete a secret, so remove that yourself:
