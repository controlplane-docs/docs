...
keywords: ['getting started', 'tutorial', 'walkthrough', 'first deploy', 'hello world', 'console ui', 'onboarding', 'beginner', 'sample app', 'cpln']
---
...
Click `Locations` in the left pane. Choose the locations where your workload will be deployed. For this quickstart, select:
...
Click `Containers` in the left pane. For the image source, select `External`, then enter:
...
Click `Firewall` in the left pane, then click the `Make Public` button at the top to allow external access.
</Step>
...
In the summary, under `Endpoints`, click the link next to `Canonical Endpoint`. Your application opens in a new tab, served from the location nearest to you.
</Step>
...
<Card title="Want your AI assistant to do the next deploy for you?" icon="robot" href="/mcp/ai-plugin" horizontal>
