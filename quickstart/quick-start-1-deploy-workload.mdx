...
Deploy your first application on Control Plane: create a Global Virtual Cloud ([GVC](/reference/gvc)), run a sample web application as a [workload](/reference/workload), and open it through a TLS-secured global endpoint. Each tab is a complete path; pick the interface you prefer.
...
In the summary, under `Endpoints`, click the link next to `Canonical Endpoint (Global)`. Your application opens in a new window, served from the location nearest to you.
</Step>
...
Whichever interface you used: once the workload reports `Ready`, its canonical endpoint serves the application:

```text
Hello World!
```

Each location also serves its own endpoint: open both from the `Deployments` page in the Console (or the `cpln workload get-deployments hello-world` output) to confirm the workload responds from AWS and GCP.
...
<Card title="Want your AI assistant to do the next deploy for you?" icon="robot" href="/ai/plugin">
