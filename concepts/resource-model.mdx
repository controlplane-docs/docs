...
description: 'Every object on Control Plane is an item of a kind, addressed by a link, scoped to an org or a GVC. One API serves the Console, the CLI, Terraform, Pulumi, and every AI agent.'
...
The path `/org/my-org/gvc/my-gvc/workload/my-app` in an API call, the `--org` and `--gvc` flags on a [CLI](/cli-reference/overview) command, and the `kind` and `name` in a [manifest](/guides/cpln-apply) are three views of the same addressing scheme. The scheme mirrors the scoping: a [GVC](/concepts/gvc) is scoped to the [org](/concepts/org), and everything created inside a GVC is scoped to that GVC. Learn it once and every interface reads the same way.

## How It Fits

| Scope | Kinds | Example link |
| --- | --- | --- |
| Org | Principals ([users](/reference/user), [groups](/reference/group), [service accounts](/reference/serviceaccount)), governance ([policies](/reference/policy), [quotas](/reference/quota)), infrastructure ([cloud accounts](/reference/cloudaccount), [agents](/reference/agent), [locations](/reference/location)), assets ([secrets](/reference/secret), [images](/reference/image), [domains](/reference/domain)), and [GVCs](/concepts/gvc) themselves | `/org/my-org/secret/my-secret` |
...
- **One API is underneath every interface.** The Console, the [CLI](/cli-reference/overview), the [Terraform](/iac/terraform) and [Pulumi](/iac/pulumi) providers, the [Kubernetes operator](/core/kubernetes-operator), and [AI agents](/ai/overview) issue the same requests, so nothing is reachable from one interface and hidden from another, and every change lands in the same [audit trail](/core/audittrail).
- **Manifests address items by kind and name.** [`cpln apply`](/guides/cpln-apply) accepts YAML or JSON declaring `kind` and `name`, creating the item when it is absent and updating it when it is not. A GVC-scoped manifest can name its target GVC in a top-level `gvc` property, which stands in for `--gvc` and the profile.
...
<CardGroup cols={2}>
  <Card title="Reference overview" icon="folder-open" href="/reference/overview">
