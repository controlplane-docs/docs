...
- **Cron**:
  - Workloads that run on a schedule and do not serve network traffic. Each execution is expected to run to completion and exit. Scheduling, concurrency, retention, retry, and run-time deadline behavior are configured under the workload's `job` spec — see [Cron Configuration](/reference/workload/types#cron-configuration).
- **Stateful**:
