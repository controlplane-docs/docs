...
The minimum and maximum number of replicas that can be deployed are configurable. Scale to zero is available only for [Serverless](/reference/workload/types#serverless) workloads using the `rps` or `concurrency` scaling strategies. When there is no traffic, serverless workloads can scale down to 0 and scale up immediately to fulfill new requests.
