...
Workloads run in Control Plane’s AWS, Azure, and GCP accounts, where clouds/regions are determined by the [GVC](/concepts/gvc) definition. Your workload may run only in a single region of one cloud, or across many regions of all the three clouds – completely up to the [GVC](/concepts/gvc) definition. Requests are routed to the nearest healthy location.
