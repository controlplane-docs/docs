...
| --- | --- | --- |
| A [secret](/reference/secret) | A [policy](/reference/policy) giving the identity `reveal` on that secret | The value, resolved from a `cpln://secret/my-secret` reference |
...
| A service that never leaves a VPC | AWS PrivateLink or GCP Private Service Connect, wired by Control Plane support | The same, over the provider's own private network |
...
- **Reading a secret takes three things, not one.** The identity is attached to the workload, an [access control](/concepts/access-control) policy grants that identity `reveal` on the secret, and the container references it as `cpln://secret/my-secret`. One key of a dictionary secret is addressed as `cpln://secret/my-secret.username`.
...
- **A private host is reached by name.** Name the host, the ports to open, and the [agent](/reference/agent) that can see it, and the workload dials that name as though it were local, on those ports only. The agent makes the connection to Control Plane itself, so the private network needs only outbound access, and the route opens without an outbound [firewall](/reference/workload/firewall) rule on the workload.
...
- **An identity can be as powerful as the cloud account allows.** Control Plane's connector attaches whatever policies the identity lists to the role it creates in your account, up to full administrator. Grant `create` and `edit` on identities to administrators only, with a [policy](/reference/policy).
