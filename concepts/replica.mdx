...
- **Stopping a replica is the restart.** `cpln workload replica stop` deletes the replica you name, and Control Plane starts a replacement to keep the count. Use it to clear a wedged process without touching the workload's other replicas.
