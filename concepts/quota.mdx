---
title: 'Quota'
description: 'The org-level cap on each Control Plane resource, from workloads per GVC to domains and agents: live usage against an allowed max, and an increase request to raise the max.'
...
A quota is a limit on how much of one resource your [org](/concepts/org) may use: a pair of numbers, `current` for what the org uses right now and `max` for the most it is allowed to reach. When `current` reaches `max`, creating more of that resource is rejected until usage drops or the max is raised. Raising a max is what a [quota increase request](/guides/manage-quotas) does: you name a quota and the new max you want, and Control Plane approves it automatically or reviews it.
...
| GVCs | Most quotas count the whole org (domains, secrets, agents, the number of GVCs). [Workloads](/concepts/workload), [identities](/concepts/identity), and [volume sets](/reference/volumeset) are counted separately in each [GVC](/concepts/gvc). |
...
| Policies | Reading quotas and submitting increase requests takes the org-level `view` [permission](/reference/quota#permissions), which the built-in org [policy](/reference/policy) grants every member. |
...
A quota increase request moves through one of three statuses.
