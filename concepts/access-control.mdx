...
<Tabs>
  <Tab title="Organization">
    Inside an [org](/concepts/org), nothing is permitted until a [policy](/reference/policy) says so. Policies grant permissions to principals through bindings, and go as narrow as one service account holding `edit` on one workload, so a principal can hold only the access its job needs.

    The creator of the org, and anyone invited as an org admin at creation, joins the built-in `superusers` group and holds every permission on every resource.
  </Tab>
  <Tab title="Billing Account">
    Billing accounts have three roles, assigned when you add someone as a billing user:
...
- **Identities are how workloads get access.** Attach an identity to a [workload](/concepts/workload) and credentials arrive at runtime, stored nowhere in the container. A group holds only users and service accounts, so an identity is always named directly in a binding.
