---
title: quota
description: "cpln quota — Display resource quotas and current usage for your organization."
keywords: ['cpln quota', 'resource quota', 'usage limits', 'org quota', 'quota check', 'quota usage', 'resource limits', 'consumption']
---
...
|57138bbd-eb31-4403-8bcb-85b9ee80395c |workloads-per-gvc     |Max number of workloads in a gvc         |builtin |1       |20   |count     |gvcLink="demo-gvc-workload"       |
|605d0bd8-1263-447c-a925-b5712bd7c5ae |cpu-per-workload      |Max CPU per container                    |builtin |0       |1000 |millicore |                                  |
|6a8a9b9d-cafa-4314-97b3-159486a0ae56 |workloads-per-gvc     |Max number of workloads in a gvc         |builtin |0       |20   |count     |gvcLink="demo-gvc-with-workloads" |
...
|fe3469cb-7be6-4372-8c13-41a083d5b469 |identities-per-gvc    |Max number of workload identities in ... |builtin |4       |20   |count     |gvcLink="demo-gvc-identity"       |
+-------------------------------------+----------------------+-----------------------------------------+--------+--------+-----+----------+----------------------------------+
```
...
```text
Commands:
  cpln quota request create <name>  Submit a quota increase request
  cpln quota request get <id>       Get a quota increase request and its status
  cpln quota request list           List the org's quota increase requests, optionally filtered by status
...
```none
cpln quota request create <name> [OPTIONS]
```
...
```text
Positionals:
  name  The name of the quota to request an increase for (e.g., gvcs).  [string] [required]

Command options:
  --requested-max  The new maximum value being requested for the quota.  [number] [required]
...
Get a quota increase request and its status
...
```none
cpln quota request get <id> [OPTIONS]
```
...
```text
Positionals:
  id  The ID of the quota increase request to retrieve (as returned by `cpln quota request create`).  [string] [required]
...
List the org's quota increase requests, optionally filtered by status
...
```none
cpln quota request list [OPTIONS]
```
...
```text
Command options:
  --status  Only list requests with this status.  [choices: "pending", "approved", "denied"]
