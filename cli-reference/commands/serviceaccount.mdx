...
description: "cpln serviceaccount — Manage service accounts used for programmatic API access."
keywords: ['cpln serviceaccount', 'service account', 'api token', 'machine user', 'ci/cd auth', 'serviceaccount create', 'service account key', 'programmatic access', 'bot user']
---
...
```text Output
{
  ...
  "created": "2026-09-15T15:30:42.928Z",
  "key": "sG5b2saERK4WjydQ.ZGVtby1jcGxuLW9yZ2FuaXphdGlvbi5kZW1vLXNlcnZpY2UtYWNjb3VudC5aTllzT1d4bTYzamtoTW12TE0zcVNwdmRIZXZ6VnNPb3F1RFdhTGtGTnVFPQ.SmwOt9f9uTtIPhkm_UKP4z_LDibC2OdurPZlqBsA9HA"
}
```
...
|demo-service-account       |demo-service-account                     |9 minutes ago |default |sG5b2saERK4WjydQ |drink="water" |
...
```bash Command
cpln serviceaccount remove-key demo-service-account --key shzWURr5fIwUDzJQ --org demo-cpln-organization
```
...
```bash Command
cpln serviceaccount tag demo-service-account-tag-1 --remove-tag drink --org demo-cpln-organization
```
...
```bash Command
cpln serviceaccount tag demo-service-account-tag-1 demo-service-account-tag-2 --remove-tag multiple-serviceaccounts --org demo-cpln-organization
```
