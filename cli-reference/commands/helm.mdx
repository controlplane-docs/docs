---
title: helm
description: "cpln helm — Install and manage Helm chart releases deployed on Control Plane."
keywords: ['cpln helm', 'helm chart', 'helm install', 'helm upgrade', 'helm release', 'chart deploy', 'kubernetes helm', 'helm registry', 'helm template']
---
...
Command options:
  ...
  --delete-leftover-resources  An upgrade or rollback that moves the release out of a GVC is refused, because it would leave its workloads, volume sets and identities behind there. Set this to delete them and proceed  [boolean] [default: false]
  --wait                       If set, will wait until all Workloads are in a ready state before marking the release as successful. It will wait for as long as --timeout  [boolean]
  --timeout                    The amount of seconds to wait for workloads to be ready before timing out. Works only if the "wait" option is set to true.  [number] [default: 300]
