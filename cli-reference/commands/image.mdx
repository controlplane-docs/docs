---
title: image
description: "cpln image — Build, push and manage container images to Control Plane and configure Docker registry authentication."
keywords: ['cpln image', 'docker push', 'container registry', 'image build', 'buildpack', 'docker image', 'cpln image build', 'cpln image copy', 'private registry', 'image tag']
---
...
[Docker](https://www.docker.com/) is required for local builds. Use `--remote` to build without Docker: the CLI uploads the source folder to the build service, which detects the build, builds the image, and pushes it to the org's private registry.
...
Command options:
  ...
  --remote                  Build remotely without local Docker — send the folder to the build service and push the image.  [boolean] [default: false]
  --detach                  With --remote, start the build and return immediately instead of waiting for it to finish.  [boolean] [default: false]
...
```bash Remote Build
# Build without local Docker. The source folder is uploaded to the
# build service and the image is pushed to the org's private registry.
cpln image build --name demo-image:1 --remote

# Start the remote build and return immediately.
cpln image build --name demo-image:1 --remote --detach
```
