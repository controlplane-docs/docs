...
keywords: ['cpln image', 'docker push', 'container registry', 'image build', 'buildpack', 'docker image', 'cpln image build', 'cpln image copy', 'private registry', 'image tag', 'remote build', 'build without docker', 'build from git repo']
---
...
Command options:
  ...
  --secret                  Secret to expose to the build in the form 'id=<id>[,src=<source>]'. src takes a local file path, or cpln://secret/<name>[.<field>] for a secret in your org. Local sources (a file path, an env variable) don't work with --remote. Repeat for each secret. Requires a Dockerfile build.
...
```bash Command
cpln image tag demo-image:1 --remove-tag drink --org demo-cpln-organization
```
