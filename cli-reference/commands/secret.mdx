---
title: secret
description: "cpln secret — Manage secrets for securely storing sensitive data such as credentials and keys."
keywords: ['cpln secret', 'kubernetes secret', 'env var injection', 'secret create', 'pull secret', 'vault', 'kms', 'secret manager', 'private key', 'cert secret', 'api key storage', 'database password']
---
...
```bash Command
cpln secret tag demo-secret-tag-1 --remove-tag drink --org demo-cpln-organization
```
...
```bash Command
cpln secret tag demo-secret-tag-1 demo-secret-tag-2 --remove-tag multiple-secrets --org demo-cpln-organization
```
