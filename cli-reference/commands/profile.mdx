---
title: profile
description: "cpln profile — Manage local CLI profiles for storing authentication and context settings."
keywords: ['cpln profile', 'cli profile', 'profile create', 'switch profile', 'profile config', 'multi-account cli', 'profile token', 'kubeconfig context equivalent', 'profile auth']
---
...
```text
Commands:
  cpln profile create <profile>       Create the referenced profile, or update it if it already exists
  ...
  cpln profile update <profile>       Manage the referenced profile
...
Create the referenced profile, or update it if it already exists
...
```none
cpln profile create <profile> --name NAME [OPTIONS]
```
...
```text
Positionals:
  profile  Name of the profile. If it does not exist, a new profile with that name will be created  [required]
