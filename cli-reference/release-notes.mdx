...
- Date: TBD (unreleased — update version and date at release)
...
- **Minor Changes**
  ...
  - Add `--remote` to [cpln image build](/cli-reference/commands/image#image-build) to build without local Docker. The CLI uploads the source folder to the build service, which builds the image and pushes it to the org's private registry. Use `--detach` to start the build and return immediately.
  - Fail [cpln image build](/cli-reference/commands/image#image-build) with a hint to use `--remote` when Docker is not installed or the daemon is not running.
  - Fix `cpln sandbox connect` SSH authentication against sandboxes that run as a non-root user.
