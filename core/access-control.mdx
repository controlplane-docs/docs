...
A “principal” is somewhat of an overloaded term representing an org item if specific kinds.
...
- [Service Accounts](/objects/serviceaccount)
...
An identity is short for workload identity. It is a reusable (can be leveraged by multiple workloads) named principal that is only used by workloads. An identity encapsulates a set of [Cloud Access](/objects/identity#cloud-access-universal-cloud-identity) rules. Cloud Access rules define least privilege policies governing access to resources in the org's configured cloud accounts. In addition to Cloud Access rules, an identity also defines a set of [Network Resources](/objects/identity#network-resources-cloud-wormhole).
