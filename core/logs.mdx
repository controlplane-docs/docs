...
The Logs UI aggregates and displays logs for running [workloads](/concepts/workload) across all configured cloud providers and regions and the logs will be aggregated and displayed as if they were running at one provider/region.

Use [filters](#log-filters) to narrow any issues that might be occurring at a specific [workload](/concepts/workload).

The [Grafana](https://grafana.com/grafana/) explorer can be used to query, visualize, and analyze log data.

## LogQL

The Control Plane logs use the LogQL query language to query your log data.

The documentation for LogQL is available in the [LogQL documentation](https://grafana.com/docs/loki/latest/logql/).

The following labels that can be used to create a query are:
...
Selecting a [location](/reference/location) or [container](/reference/workload/containers) automatically add the value to the LogQL query.
...
This query will produce a count of inbound requests grouped by source ip address over the last 10 minutes.
...
The log retention period for logs stored for 30 days by default and can be adjusted for each Org.
