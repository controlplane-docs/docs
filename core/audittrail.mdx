...
Every action is captured, securely stored, and indexed, whichever interface performed it. The Console, the [CLI](/cli-reference/overview), [Terraform](/iac/terraform), [Pulumi](/iac/pulumi), the [Kubernetes operator](/core/kubernetes-operator), and AI agents on the [MCP server](/ai/mcp) all call the same [API](/api-reference/api), so none of them can act outside the trail. In the UI, most resources include an audit trail link that opens the audit trail page with the resource ID prefilled. Additional filters can be added to drill down to specific events.
