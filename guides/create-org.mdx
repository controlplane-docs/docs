---
title: Create an Org
description: Create a new organization using the Console UI or CLI, including prerequisites for the org_creator or billing_admin role.
...
4. If the user is already on the current users list and they don't have the `org_creator` or `billing_admin` role, Click the dropdown for the corresponding to the user, select the `org_creator` or `billing_admin` role and click `Update`.
...
2. Enter a unique Org name, optional description, and any additional org admin email addresses.
3. click `Create Org`. The user creating the role, and any additional org admins, will automatically be assigned to the `superusers` group.
