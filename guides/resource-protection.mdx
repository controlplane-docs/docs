...
description: Prevent accidental deletion of critical resources by applying the cpln/protected tag using the Console UI, CLI, or YAML manifests.
...
The Control Plane platform will prevent the deletion of resources with the `cpln/protected` [tag](/core/misc#tags) set to `true`.
...
### Remove Tag
  ...
  1. Click `Tags` in the left pane.
  2. Click the delete icon button corresponding to the row that contains the `cpln/protected` tag.
  3. Click `Update`.
...
Follow the examples below that target a workload to add and remove the `cpln/protected` tag. Substitute `workload` with the target resource kind.
