...
A GVC endpoint routes traffic across all domains mapped to the same GVC and is useful when a single CDN route is configured for many domains or wildcard subdomains. GEO routing still applies, but traffic will be sent to all locations even if a specific workload in one location is unavailable.
