...
description: Set up a Content Delivery Network such as Cloudflare or Amazon CloudFront to protect and accelerate your workloads on Control Plane.
...
- Using the certificate and private key generated in [Step One - Certificate Generation Set Up](#certificate-generation-set-up), create a new [TLS Secret](/reference/secret#tls) at Control Plane by performing the following:
  ...
  - Click `Data` in the left menu. Either upload or paste the respective certificate and private key file in the proper textbox. The TLS Chain can be left empty since this certificate is self-signed.
...
- Click the `New` button at the top.
- Click `Advanced` in the left menu. Enter the `Fully Qualified Domain Name (FQDN)` of your Domain. You will need to prove ownership of the domain before continuing.
- Select `CNAME` for the `DNS Mode`.
- Select and configure the desired `Routing Mode`.
- Click the `Add +` tab, and toggle the `Use Custom Server Certificate` and set the `Server Certificate Authority PEM` to the [TLS Secret](/reference/secret#tls) created in the step above then click `Create`.
