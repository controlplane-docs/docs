...
The cpln [profile](manage-profile) named `default` will be used by Docker when authenticating to the private registry before the pushing
...
In situations where the [CLI](/reference/cli) cannot be used, authentication to your private [image registry](/reference/image) is achieved by using a [Service Account](/guides/create-resources/create-service-account).
...
- Password: `Service Account Key`
  <Note>
    Refer to the [Create a Service Account](/guides/create-resources/create-service-account) guide for instructions on how to generate a new
    key.
  </Note>
