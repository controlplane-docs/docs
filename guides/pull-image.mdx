...
3. Google Container Registry [deprecated]: `gcr.io/REGISTRY/IMAGE_NAME:TAG`
4. Google Artifact Registry: `LOCATION-docker.pkg.dev/PROJECT-ID/REPOSITORY/IMAGE_NAME:TAG`
5. GitHub Container Registry: `ghcr.io/OWNER/IMAGE_NAME:TAG`
...
3. From the `Secret Type` dropdown, select one of the following:
   - Create a new [Docker](/reference/secret#docker) secret for images stored at:
     ...
     - [Google Artifact Registry](/reference/secret#google-artifact-registry).
...
3. After creating or saving the [workload](/reference/workload), a new deployment will be queued. Click on the `Deployments` link to view the deployment status. If there is an error (i.e. authentication failed), it will be displayed under each deployment.
