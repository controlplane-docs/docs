...
description: "CI/CD integration examples for GitHub Actions, GitLab CI, Bitbucket Pipelines, and CircleCI. Covers building, testing, and deploying with Terraform or CLI."
keywords: ['gitops', 'pipeline', 'jenkins', 'azure devops', 'travis', 'argocd', 'flux', 'auto deploy', 'workflow yaml', 'deploy automation', 'pull request deploy']
---
...
The command can accept a YAML file containing multiple resources. Each resource must be separated using `---`. Resources in a single `cpln apply` invocation are applied in dependency order automatically (e.g., a [GVC](/reference/gvc) is applied before the [workload](/reference/workload) that references it), so related resources can share a file in any order.

If the name of an existing resource is changed, the `cpln apply` command will create a new resource. Any orphaned resources will need to be manually deleted.
