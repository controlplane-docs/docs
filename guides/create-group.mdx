...
3. Click `Users` in the left pane. Click `Add User` to view a list of [users](/reference/user) will be shown that can be selected. A [query](/reference/group#query-rules) can also be defined to dynamically assign users to the group. Click `Ok` to continue.
4. Click `Service Accounts` in the left pane. Click `Add Service Accounts` to select a [service accounts](/reference/serviceaccount).
