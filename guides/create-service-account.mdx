...
1. Create a new service account by either:
   - Click `Service Accounts` in the left menu and click `New`, or
...
3. `Optional:` Select a group for the service account to inherit that group's permissions.
3. Optionally, click `Tags` and enter any [tags](/core/misc#tags).
4. Click `Create`.
5. Click the `Keys` in the left pane to generate a new token that is used to create a profile.
6. Enter a optional key description and click `Add`. Copy or download the generated key.
