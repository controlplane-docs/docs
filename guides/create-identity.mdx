...
1.  Create a new identity using one of the following methods:
    - Click `Identities` in the left menu and click `New`, or
...
4.  Under `Cloud Wormhole` in the left pane, configure private network connectivity. This step is optional. See [Network Resource](#network-resource) for additional details.
...
7.  Click `Create`. The identity has been successfully created and the identity info page will be shown. This identity will now be available
...
Verify that the policies selected are correct and click `Create`. If a new AWS role was selected, Control Plane will provision a new role in AWS that will be named the same as the `Object Name` shown in the `Info` page of the [identity](/reference/identity).
