...
description: Create an identity within a GVC to configure cloud access rules, network resources, and native networking for your workloads.
...
3.  Under `Cloud Access` in the left pane, select the cloud provider (`AWS`, `Azure`, `GCP`, or `NATS`) to configure a cloud access rule. Requires at least one
...
6.  Click `Tags` in the left pane, and enter any optional [tags](/core/misc#tags). Click `Create`.
...
Tunneling network traffic from [workloads](/reference/workload) to specific TCP hosts and ports is facilitated using [agents](/reference/agent) deployed within the private network. This capability is referred to as “Cloud Wormhole”.
...
<Info>A maximum of **5** ports can be added per resource.</Info>
