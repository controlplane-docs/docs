...
3.  Under `Cloud Access` in the left pane, select the cloud provider (`AWS`, `Azure`, `GCP`, or `NATS`) to configure a cloud access rule. Requires at least one [cloud account](/reference/cloudaccount) for the chosen provider. This step is optional. See [Cloud Access](#cloud-access) for details.
...
6.  Optionally, select `Tags` from the left pane to add [tags](/core/misc#tags).
7.  Click `Create`.
8.  The identity has been successfully created and the identity info page will be shown. This identity will now be available
