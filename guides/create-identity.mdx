...
4. Under `Cloud Wormhole` in the left pane, configure private network connectivity. This step is optional. See [Cloud Wormhole](#cloud-wormhole) for additional details.
...
To set up an AWS cloud access policy, select `AWS` under `Cloud Access`.
...
1. Select a registered [agent](/reference/agent) matching the environment you want to access.
