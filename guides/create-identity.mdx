...
3. Under `Cloud Access`, select the cloud provider (`AWS`, `Azure`, `GCP`, or `NATS`) to configure a cloud access rule. Requires at least one
...
4. Under `Cloud Wormhole`, configure private network connectivity. This step is optional. See [Cloud Wormhole](#cloud-wormhole) for additional details.
...
5. Under `Native Networking`, configure cloud-native private connectivity. This step is optional. See [Native Networking](/guides/native-networking/native-networking-setup) for details.
