...
3. Optional: Click `Tags` in the left resource menu. Enter any optional [tags](/core/misc#tags).
4. Click `Create` and the console will display the bootstrap config JSON text.
5. `Optional:` Copy or Download the bootstrap config JSON. If you copy the `UserData script`for AWS and Azure or the `Startup script` for GCP in the next step, you can skip this step.
6. Click `Next` and copy the `UserData script` for deploying an agent in AWS and Azure or copy the `Startup script` for GCP. Click `Done`.
