...
description: Define an agent and generate a bootstrap config file for installing the agent in your internal environment or at a cloud provider.
...
4. Click `Create`, the console will display the bootstrap config JSON. Manually save the JSON or click `Download Config File`. You don't need to save the JSON if you click `Next` and save the provisioning script. There you will find a `User Data script` for AWS and Azure, and a `Startup script` for GCP. Download whichever applies to your cloud provider, as it will install and configure the agent on your VM at upon startup.
