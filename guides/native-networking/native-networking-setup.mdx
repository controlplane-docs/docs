...
<Note>
You can configure multiple Native Networking connections to different resources by specifying different port numbers for each resource. If the same port is required, a new PrivateLink/Private Service Connect endpoint must be configured.
</Note>

6. Select a `Cloud Account` (only needed if using the browser service to look up the endpoint service name).
7. Enter the appropriate endpoint service name (`Service name` for *AWS* and `Service attachment name` for *GCP*).
