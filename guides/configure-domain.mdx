...
description: "Configure custom domains for workloads with DNS verification, routing modes, TLS certificates, and path-based routing via Console UI and CLI manifests."
keywords: ['apex domain', 'cname', 'subdomain setup', 'lets encrypt', 'host header', 'wildcard domain', 'route53', 'cloudflare dns', 'dns delegation', 'fqdn', 'path routing', 'subdomain routing']
---
...
The [custom domain quickstart](/quickstart/quick-start-3-custom-domain) walks through the Console form for an apex domain: the TXT record that proves ownership, the CNAME record that points the domain at the GVC, and the refusal the Console answers with until the TXT record propagates.
...
The [custom domain quickstart](/quickstart/quick-start-3-custom-domain) shows the Console form step by step, from the records the domain needs to the workload it routes to.
...
<AccordionGroup>
  ...
  **Note:** The GVC_ALIAS can be obtained from the console UI by clicking on the GVC and it will be displayed on the `Info` page. Using the CLI, executing the command `cpln gvc get GVC_NAME` will output the alias.
...
### Domain Additional Settings
  ...
  - The [accept all hosts](/reference/domain#acceptallhosts) or [accept all subdomains](/reference/domain#acceptallsubdomains) settings (`acceptAllHosts` in the above example manifest) can be enabled when dedicated load balancing is enabled.
...
- When using Path Based Routing:
  ...
  - When dedicated load balancing is enabled, a [host prefix](/reference/domain#hostprefix) (`hostPrefix` in the above example manifest) can be optionally defined.
