...
The [custom domain quickstart](/quickstart/custom-domain) walks through the Console form for an apex domain: the TXT record that proves ownership, the CNAME record that points the domain at the GVC, and the refusal the Console answers with until the TXT record propagates.
...
The [custom domain quickstart](/quickstart/custom-domain) shows the Console form step by step, from the records the domain needs to the workload it routes to.
