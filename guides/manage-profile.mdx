...
description: Create and manage CLI profiles for user and service account authentication, including interactive login and token-based configuration.
keywords: ['profile token', 'kubectl context', 'aws profile', 'multi-account cli', 'switch org', 'switch profile', 'default profile', 'set-default', 'service account profile']
---
...
will print a login URL and a six-digit confirmation code. Open the URL in a browser on any device, sign in, and enter
the code. If the authentication is successful, a new profile named `default` will be created (if it doesn't already
exist). This login targets only the profile named `default`. After logging in, this profile will be marked as the
default and used by any future CLI command.
...
will print a login URL and a six-digit confirmation code to complete sign-in in any browser. If the authentication is
successful, a new profile with the given name will be created (if it doesn't already exist). After logging in, this
profile will be marked as the default and used by any future CLI command.
...
Future CLI commands will use this profile. If the profile is not authenticated, follow the [Interactive Login for Custom Profile](#interactive-login-custom-profile) instructions.
