...
A Domain can be created / updated using the [CLI's apply](/guides/cli/cpln-apply) command.

Below are a few sample Domain manifests (in YAML) that can be used as input to the [CLI's apply](/guides/cli/cpln-apply) command.
...
<Warning>
  Before using the [CLI's apply](/guides/cli/cpln-apply) command, the [TXT DNS records](#required-dns-records) must exist and be propagated
