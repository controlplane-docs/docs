...
Follow the [Create an Agent](/guides/create-resources/agent) guide to define an agent and generate the bootstrap config file that will be used in
...
15. Expand the `Advanced details` section. Scroll to the bottom and paste the contents
    of the JSON payload generated in [step one](#step-one-create-an-agent) within the `User data` textbox.
