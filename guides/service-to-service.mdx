...
keywords: ['east-west', 'internal traffic', 'cpln.local', 'private endpoint', 'inter-workload', 'service mesh', 'mtls', 'zero trust calls', 'workload dns']
---
...
2. Click `Firewall` in the left pane, and click on the `Internal` tab.
...
After updating, the rule will be enforced within a minute and a new deployment of the [workload](/reference/workload) will be queued.
