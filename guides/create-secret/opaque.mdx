---
title: Opaque
description: Create an opaque secret to store any text-based sensitive data such as API keys, tokens, or configuration values.
keywords: ['generic secret', 'string secret', 'plain text secret', 'stripe key', 'twilio key', 'sendgrid key', 'env var secret', 'k8s opaque']
---
...
- **Build Credentials**: A private registry token an image build needs, mounted with [`cpln image build --secret`](/cli-reference/get-started/images#build-secrets) so it never lands in the image
...
<Tip>
For a short value, skip the file and pipe it to `--file -`:

```bash
printf '%s' 'sk_live_abc123xyz789' | cpln secret create-opaque \
  --name my-api-key \
  --file - \
  --encoding plain \
  --org my-org
