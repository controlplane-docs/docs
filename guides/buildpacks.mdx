...
keywords: ['cnb', 'pack cli', 'heroku buildpack', 'paketo', 'auto detect language', 'no dockerfile', 'language detection', 'image build', 'cpln image build', 'oci builder']
---
...
<Note>
`--build-arg` and `--secret` belong to Dockerfile builds. Buildpacks have no `ARG` mechanism and no secret mount, so a buildpack build warns about any build argument it was given and rejects `--secret` outright. Pass build-time values with `--env` or `--env-file` instead, keeping in mind that a buildpack can persist them into the image, so they are not a place for credentials.
</Note>
