...
1. Using the console:
   ...
   - [Create a Service Account](/guides/create-resources/create-service-account) whose key will be used in step #2.

   - Create one or more [policies](/guides/create-resources/policy) granting the [service account](/reference/serviceaccount) permissions to resources (GVCs, workloads, etc.) as required.
...
This [group](/reference/group) will give full access to the [service account](/reference/serviceaccount). If the CLI commands are now returning as expected, additional permissions will need to be added to the [service account](/reference/serviceaccount) using [policies](/guides/create-resources/policy).
