...
The [cpln delete](/api-reference/cli#delete) command serves as the opposite of the CLI [apply](/api-reference/cli#apply) command. While the `apply` command creates and updates resources, the `delete` command deletes resources. Both the `apply` and the `delete` commands accept the same options.
...
To delete an [Identity](/api-reference/identity), a [Volume Set](/api-reference/volumeset) or a [Workload](/api-reference/workload) resource you need to specify a GVC in one of the following methods.

- Specify a `gvc` within your [cpln profile](manage-profile). This will add the `gvc` to the session context of the profile and will be refered to as the default `gvc` when executing any future command including the `cpln delete` command.
...
[Click here](/api-reference/cli#delete) to view the CLI reference page for the `delete` command.

### [Delete an Applied K8s File](#delete-k8s)
...
The `delete` command will use the logic of the CLI [convert](/api-reference/cli#convert) command and then delete the resources defined in the output.

### [Delete from Standard Input](#delete-stdin)
...
A file or an input containing an [Identity](/api-reference/identity), a [Volume Set](/api-reference/volumeset) or a [Workload](/api-reference/workload) resource will be deleted in the scope of the specified `gvc` in the cpln apply modal. In case a `gvc` is defined within a resource, the resource will be deleted in the scope of that `gvc`.
