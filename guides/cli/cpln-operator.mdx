...
- [install](#install-command): Deploys the operator by creating (or reusing) a service account, assigning it to a group, generating a key, and creating a Kubernetes secret in the `controlplane` namespace.
...
Deploys the Kubernetes operator to your cluster.
...
Removes the Kubernetes operator from your cluster by deleting the operator-installed secret in the `controlplane` namespace.
