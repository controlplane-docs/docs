---
title: Rate Limiting
description: Configure rate limiting for workloads using the Envoy Rate Limit project with a Redis backend deployed on Control Plane.
keywords: ['throttle', 'requests per second', 'qps cap', 'api throttle', 'token bucket', 'leaky bucket', 'envoy ratelimit', 'redis rate limit', 'ddos protection', 'request quota']
---
