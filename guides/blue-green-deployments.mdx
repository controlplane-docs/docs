...
<Tabs>
  ...
  <Tab title="CLI Patch (Quick Cutover)">
    ...
    ```bash
    echo '{"spec":{"ports":[{"number":443,"routes":[{"prefix":"/",
      "workloadLink":"//gvc/my-gvc/workload/my-app-green","port":8080}]}]}}' | \
      cpln rest patch /org/ORG_NAME/domain/DOMAIN_NAME --file -
    ```
  </Tab>
  <Tab title="Console UI">
...
```bash
cpln logs '{gvc="my-gvc", workload="my-app-green"}' --org ORG_NAME --tail
```
...
jobs:
  deploy:
    ...
    steps:
      ...
      - name: Deploy green workload and wait for it to be ready
        run: |
          sed 's|IMAGE_TAG|'"$IMAGE_TAG"'|g' green-workload.yaml | \
            cpln apply --file - --org $CPLN_ORG --ready
...
| View workload logs | `cpln logs '{gvc="GVC", workload="my-app-green"}' --org ORG --tail` |
...
<CardGroup cols={2}>
  ...
  <Card title="GVC (Global Virtual Cloud)" href="/concepts/gvc" icon="/icons/gvc.svg">
