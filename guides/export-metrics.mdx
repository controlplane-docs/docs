...
2. **Generate a Key:**
   - Click `Keys` in the left pane. Create a new key for the `prometheus-federate` Service Account and save it securely.

3. **Create a Policy:**

Grant the Service Account `readMetrics` permission by creating a policy using one of the following methods:

**Using the Control Plane Console UI:**

1. Create a policy named `prometheus-federate` that grants `readMetrics` permission to the Service Account.
2. Navigate to the Policies section in the Control Plane Console.
3. Click on `New +` and fill in the details:
   - **Name**: `prometheus-federate`
   - **Description**: `prometheus-federate`
4. Click `Target` in the left pane.
   - **Target Kind**: `org`
5. Click `Binding` in the left pane.
6. Click `Add Binding`:
