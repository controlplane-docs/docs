...
2. **Generate a Key:**
   - Click `Keys` in the left resource menu. Create a new key for the `prometheus-federate` Service Account and save it securely.
...
- Click `Target` in the left resource menu.
  - **Target Kind**: `org`
- Click `Binding` in the left resource menu.
- Click `Add Binding`:
