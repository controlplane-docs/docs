...
- Specify a `gvc` within your [cpln profile](/guides/manage-profile). This will add the `gvc` to the session context of the profile and will be referred to as the default `gvc` when executing any future command including the `cpln delete` command.
