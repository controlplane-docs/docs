...
keywords: ['prometheus metrics', 'rps', 'qps', 'latency p95', 'response time', 'cpu utilization', 'memory utilization', 'gpu usage', 'storage metrics', 'metric names', 'quota utilization', 'network traffic', 'egress cost', 'cross zone', 'public ip', 'data transfer']
---
...
- `egress`: Traffic leaving the workload's cloud region — the public internet, a cloud resource in another region, or another Control Plane location (bytes)
- `cross_zone_traffic`: Traffic between Control Plane workloads that crosses an availability zone inside one region (bytes)
- `cross_zone_external_traffic`: Traffic between a workload and an endpoint that is in the same cloud region but outside Control Plane, reached over a public IP (bytes)
