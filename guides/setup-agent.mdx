...
2. Log in to the [AWS Console](https://console.aws.amazon.com). Once logged in, select the AWS Region at the navigation bar in the top-right corner. Choose the same region where the services it will be accessing are located.
...
7. For `Instance type` in production environments, refer to the [Agent Sizing Guidance](/reference/agent#agent-sizing-guidance) to select the appropriate instance type for running an agent. Select `t3.small` or `t3.medium` for testing purposes. An instance with at least 2 vCPUs and 4 GiB of memory is recommended for optimal performance.
...
### Step Three - Create a Auto Scaling Group in AWS
...
- `Note:` Confirm your subnet has Auto-assign public IPv4 address by navigating back to `VPC` and clicking `Subnets` in the AWS console. If it is not enabled, you can also expand `Advanced network configuration` and click `Add network interface` set `Auto-assign public IP` to `Enable`.
...
8. For `Size` in production environments, refer to the [Agent Sizing Guidance](/reference/agent#agent-sizing-guidance) to select the appropriate instance type for running an agent. Select `Standard D2s v3` for testing purposes. An instance with at least 2 vCPUs and 4 GiB of memory is recommended for optimal performance.
...
13. Select an existing load balancer or click `Create a load balancer`, then click `Next: Management`.

14. Under the `Management` section, enable Basic plan for free, set Boot diagnostics to Enable with managed storage account.

15. Click `Next: Health`, skip the section, then click `Next: Advanced`.

16. Under the `Advanced` section, paste the `Userdata Script` copied in Step 1 into the `Custom Data` field, then click `Create`. If you created a new key pair, click `Download private key and create resource`.
...
### Step Two - Create a Instance Template in GCP

1. If you already have the `Startup script`, you can skip this step. In the Control Plane UI, click `Agents` in the left menu, select your agent, then choose `Download Scripts` from the `Actions` menu. Paste your bootstrap token and copy the YAML that's shown in the `Startup script` tab. The `Startup script` is used to automatically install and configure the Control Plane agent on your VM at first boot.
...
5. Under the `Location` section, select `Regional`, and choose your preferred region. The region needs to match where your internal resources are located, as the agent must be in the same VPC and region to connect to them.

6. For the `Machine configuration`, in production environments, refer to the [Agent Sizing Guidance](/reference/agent#agent-sizing-guidance) to select the appropriate instance type for running an agent. Select `C4` for testing purposes.
...
### Step Three - Create a Instance Group in GCP
...
4. Under the `Location` section, select `Multiple zones`, choose your preferred regions.
...
6. Under the `Autoscaling` section, click `Configure Autoscaling` and set the `Autoscaling Mode` to `On`, and update the Minimum and Maximum number of instances as needed, and add any additional autoscaling signals if required. For production, set Minimum to 2 and Maximum to at least 4. For testing, set Minimum and Maximum capacity to 1.

- `Note:` You can also scale down to 0 or scale up to multiple agents for redundancy and reliability.

7. `Optional:` Under the `Autohealing` section, you can add a Health check to automatically recreate VM instances that fail the health check. Set `on failed Health check` to `Repair Instance`. Set the `Updates during VM instance repair` to `Update instance configuration` so that repaired VMs are recreated using the latest instance template.

8. Click `Create` in the bottom-left corner.
...
3. Within a few minutes agent pods will be ready to handle requests.
