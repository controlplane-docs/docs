...
1. If you already have the `Startup script`, you can skip this step. In the Control Plane UI, click `Agents` in the left menu, select your agent, then choose `Download Scripts` from the `Actions` menu. Paste your bootstrap token and copy the YAML that's shown in the `Startup script` tab.
...
3. For the Instance template, select the name you created in Step Two,(e.g., `cpln-agent-instance-template`) and click `Next`.

4. Under the `Location` section, select `Multiple zones`, choose your preferred regions.

5. For the Target distribution shape, select `Any` to evenly distribute instances across all selected zones, and check the `Allow instance redistribution` checkbox.

6. Under the `Autoscaling` section, update the Minimum and Maximum number of instances as needed, and add any additional autoscaling signals if required.  For production, set Minimum to 2 and Maximum to at least 4. For testing, set Minimum and Maximum capacity to 1.
...
2. The agent will now be running, connecting to the Control Plane servers, and ready to process requests. To verify, go to the Control Plane UI, click `Agents` in the left menu, select your agent, and you will see a green heartbeat pinged recently. It may take up to a minute to appear after the agent starts.
