...
8. For `Size` select `Standard D2s v3` for testing purposes. In production environments, refer to the [Agent Sizing Guidance](/reference/agent#agent-sizing-guidance) to select the appropriate instance type for running an agent.  An instance with at least 2 vCPUs and 4 GiB of memory is recommended for optimal performance.
...
6. Under the `Autoscaling` section, click `Configure Autoscaling` and set the `Autoscaling Mode` to `On`, and update the Minimum and Maximum number of instances as needed, and add any additional autoscaling signals if required.

- For testing, set Minimum and Maximum capacity to 1.
- For production, set Minimum to 2 and Maximum to at least 4.
- `Note:` You can also scale down to 0 or scale up to multiple agents for redundancy and reliability.

7. `Optional:` Under the `Autohealing` section, you can add a Health check to automatically recreate VM instances that fail the health check. Set `on failed Health check` to `Repair Instance`. Set the `Updates during VM instance repair` to `Update instance configuration`. So that repaired VMs are recreated using the latest instance template. (Should remove for consistency)
