...
1. Prepare a the UserData by going to your agent's page in ControlPlane. Select `Download Scripts` from the `Actions` menu. Paste the bootstrap token you saved earlier and copy the shell script shows in the `Startup Script` tab.

1. Deploy a VM (or an autoscaling group) from your preferred Linux base image.

2. During configuration of the VM or the launch template set the `startup-script` metadata to she script you produced in step 1. See [Metadata keys for Linux startup scripts](https://docs.cloud.google.com/compute/docs/instances/startup-scripts/linux)
