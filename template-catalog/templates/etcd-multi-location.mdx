...
It is also the consensus store used by the `postgres-multi-location` template, and is independently useful as a cross-region coordination store for your own services.
...
The template asks the platform to restart only one member at a time, but that setting is not retained by the API, so no limit is actually in force. Running more members per location has not been tested and should not be assumed to help.
