...
Auto-compaction is enabled by design (periodic, every hour). The first compaction happens an hour after a member starts, and a restart resets that clock and is not configurable. Without it, a continuously written cluster grows revisions until it reaches etcd's 2 GiB backend quota and goes read-only. Compaction reclaims pages for reuse rather than shrinking the file, so the reported `dbSize` plateaus instead of dropping — only `etcdctl defrag` returns space to the filesystem.
