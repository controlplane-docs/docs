---
title: 'Elasticsearch'
icon: /template-catalog/templates/icons/elasticsearch.png
description: 'Deploy an Elasticsearch 8.17 cluster on Control Plane using the Template Catalog. Covers Kibana, cluster and resource settings, scaling, and SLM snapshot backups to AWS S3 or GCS, with restore scenarios.'
keywords: ['lucene', 'full-text search', 'indices', 'shards', 'master election', 'snapshot lifecycle management', 'disaster recovery', 'search engine', 'analytics', 'elastic', 'stateful workload', 'volume set']
---
...
```bash
cpln port-forward RELEASE_NAME-kibana 5601 --gvc GVC_NAME
# Then open http://localhost:5601 in your browser
```

**Port-forward to Elasticsearch:**
```bash
cpln port-forward RELEASE_NAME-elasticsearch 9200 --gvc GVC_NAME
```
