---
title: CDC Pipeline
icon: /template-catalog/templates/icons/cdc-pipeline.png
...
The first thing to do after installing is **replace every `change-me-cdc-pipeline-*` placeholder value** with real secrets. The template validates at deploy time that shared credentials are consistent — mismatches are caught before any workload starts.
