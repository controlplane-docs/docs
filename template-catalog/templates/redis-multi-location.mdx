...
The backup produces a single file (`redis-<timestamp>.rdb.gz`). Download and decompress it, then copy it over `/data/dump.rdb` on the replica you want to restore and restart that replica — the server loads `dump.rdb` from its data directory at start.
