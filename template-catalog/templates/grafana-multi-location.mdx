...
description: Deploy Grafana OSS across several Control Plane locations behind one endpoint, with dashboards, users, sessions and datasource credentials shared through a stretched Patroni PostgreSQL cluster, and every firing alert notified exactly once — by one pinned evaluator or Redis-coordinated alerting HA.
...
A firing alert is always **notified exactly once**, in one of two shapes you choose with `alerting.highAvailability.enabled`:
