...
| Grafana-only change (for example `replicas: 1` → `3`) | **Zero downtime** — 1160 probe samples across four endpoints during the rollout, no failures |
| Any `helm upgrade` that touches the database tier | Database writes unavailable in **every** location for about **117 s**, and the Grafana tier returns `503` for part of that window |
...
<Warning>
The backup path has been exercised against **AWS S3** only, and no restore has been performed by us. The `gcp` and `minio` providers are configured the same way but have not been exercised — validate them in a test install, and rehearse your restore procedure, before relying on either.
</Warning>
...
- **Every `helm upgrade` that touches the database tier is a planned outage** of roughly two minutes, during which both Grafana tiers return errors. Changes confined to the Grafana workloads roll with zero downtime.
