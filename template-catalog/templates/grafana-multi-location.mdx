...
<Note>
**A `Failed to lock database` line during a cold start is expected, and it clears itself.** Grafana runs its schema migrations under a non-blocking lock with no retry, so an instance that arrives while another holds the lock exits; the platform restarts it and the next attempt succeeds against the already-migrated schema. The chart staggers the UI instances so they do not arrive together, which removes these restarts at the default one replica per location. Two replicas in the **same** location still start together, so at `replicas: 2` expect under one self-healing restart per extra replica — a three-location run at that setting measured **two**. They are far more numerous when the database primary does not land in `postgresML.primaryLocation`, because the migrations then run cross-region and instances keep restarting while they finish — **15** in that case, against 2 with the primary in place. Nothing needs doing either way.
</Note>
