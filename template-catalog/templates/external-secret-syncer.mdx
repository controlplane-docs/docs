...
description: Deploy External Secret Syncer on Control Plane using the Template Catalog. Covers configuration, scaling, and syncing secrets from AWS, Vault, 1Password, Doppler, and Infisical.
keywords: ['ess', 'secret sync', 'aws secrets manager', 'azure key vault', 'gcp secret manager', 'hashicorp vault', 'secret rotation', 'ssm parameter store', 'external secrets operator', 'secret refresh', 'infisical']
---
...
essConfig:
  ...
  secrets:
    ...
    # - name: gcp-project
    #   provider: my-gcp
    #   dictionaryFromProject: true   # syncs every accessible secret from the GCP project
