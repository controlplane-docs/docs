...
description: Deploy External Secret Syncer on Control Plane using the Template Catalog. Covers configuration, scaling, and syncing secrets from AWS, Vault, 1Password, and Doppler.
keywords: ['ess', 'secret sync', 'aws secrets manager', 'azure key vault', 'gcp secret manager', 'hashicorp vault', 'secret rotation', 'ssm parameter store', 'external secrets operator', 'secret refresh']
---
