...
workload:
  name: ess
  image: ghcr.io/controlplane-com/cpln-build/external-secret-syncer:v1.3.0
  resources:
    cpu: 200m
    memory: 256Mi
  port: 3004
...
| GCP Secret Manager | `gcpSecretManager.projectId`, `gcpSecretManager.credentials.clientEmail`, `gcpSecretManager.credentials.privateKey` |

AWS and GCP providers optionally accept the credentials. If omitted, ESS falls back to credentials provided through the workload's cloud access [identity](/reference/identity).
