...
- **Stateful Patroni PostgreSQL Workload** — A Patroni-managed PostgreSQL cluster with configurable replica count and resources. Each replica has its own volume.
...
Connect using the appropriate endpoint depending on which optional components are enabled:

| Setup | Host |
|-------|------|
| PgBouncer enabled | `RELEASE_NAME-pgbouncer.GVC_NAME.cpln.local` |
| Proxy only | `RELEASE_NAME-postgres-ha-proxy.GVC_NAME.cpln.local` |
