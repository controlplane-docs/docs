...
icon: /template-catalog/templates/icons/postgresql.png
...
**Confirm you have a base backup.** WAL archiving on its own cannot restore a cluster — it replays *from* a base backup. If you run `wal-g` backups, list what is actually in your bucket before relying on them:

```bash
cpln workload exec RELEASE_NAME-postgres-ha --gvc GVC_NAME --container wal-g-backup \
  -- wal-g backup-list
```
