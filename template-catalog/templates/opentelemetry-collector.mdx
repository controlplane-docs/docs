...
Steps 1 is completed while deploying the template. After deploying the template, complete the following steps to finish configuration:

**1. Normalize URL patterns (recommended)**

If your application has dynamic URL segments, use the `transform` processor to normalize them into fixed patterns. This keeps span cardinality manageable and makes your traces queryable. In both configs, replace `PLACEHOLDER` with each path segment you want to collapse:
...
**2. Installing OpenTelemtetry SDK to your application**
