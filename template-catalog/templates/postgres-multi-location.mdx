...
<Warning>
The backup path has been exercised against **AWS S3** only: dumps and WAL/base backups are written and verified in the bucket. **No restore has been performed by us** — the procedures below are documented, not tested. The `gcp` and `minio` providers are configured the same way but have not been exercised — validate them in a test install before relying on them. Likewise, rehearse your restore procedure in a scratch environment before you need it.
</Warning>
