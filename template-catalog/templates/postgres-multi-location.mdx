...
<Warning>
**Version 1.0.2 renamed every PostgreSQL-side resource, and there is no in-place upgrade path.** The `-ml` infix is gone: the workloads, secrets, identity, policy and — critically — the **volume set** are now named `{release}-postgres…` instead of `{release}-postgres-ml…`. Running `helm upgrade` from 1.0.0 or 1.0.1 provisions a **new, empty volume set** and leaves the old one behind, still holding your data and still billing. The upgrade reports success and the cluster comes up healthy with no data in it.
</Warning>
...
The bundled etcd cluster's resources were never renamed and are unaffected.

Migrate with a backup and restore rather than an upgrade:

<Steps>
  <Step title="Back up the existing cluster">
    Use `backup.mode: logical`, or run a `pg_dumpall` against the proxy endpoint of the old release.
  </Step>
  <Step title="Uninstall the old release">
    `cpln helm uninstall RELEASE_NAME --gvc GVC_NAME`. Confirm afterwards which volume sets remain.
  </Step>
  ...
  <Step title="Delete the orphaned volume set">
    Once you have verified the restored data, delete the leftover `{release}-postgres-ml-vs`. It is billed until you do.
  </Step>
</Steps>
...
- **Do not `helm upgrade` a 1.0.0 or 1.0.1 cluster to 1.0.2.** Resource names dropped the `-ml` infix in 1.0.2, so the upgrade succeeds, comes up healthy and is **empty** — your data stays on the old volume set, which is orphaned and still billing. Back up, uninstall, reinstall and restore instead: see [Upgrading From 1.0.0 or 1.0.1](#upgrading-from-1-0-0-or-1-0-1).
