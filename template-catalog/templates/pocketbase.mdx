---
title: PocketBase
icon: /template-catalog/templates/icons/pocketbase.png
...
<Warning>
**Every realtime connection is closed after ten minutes.** The platform cuts any HTTP request at the workload's `timeoutSeconds`, which this template already sets to `600` — the maximum the platform allows. Server-Sent Events are not exempt the way an upgraded websocket connection is, so ten minutes is the longest a subscription can live here. **This is a hard ceiling, not a tunable.**

A stream that goes quiet ends sooner: PocketBase itself disconnects an idle subscription after about five minutes, by its own design. So ten minutes is the ceiling for an *active* stream, not a floor.
</Warning>
...
| A hand-rolled `EventSource`, `curl`, or custom consumer | **Handle the reconnect yourself.** Expect an active stream to end at ten minutes at the latest — sooner if it goes idle — and resubscribe when it does. |
