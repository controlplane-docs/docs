...
- **Identity & Policy** — An identity bound to the workload with `reveal` access to the config secret only, plus keyless cloud access scoped to exactly your bucket (AWS and GCP backends).
  ...
  <Tab title="Google Cloud Storage (keyless)">
    ...
    <Steps>
      ...
      <Step title="Confirm bucket access">
        No policy to author — the template grants the workload identity `roles/storage.objectAdmin` scoped to the bucket automatically. Mimir authenticates via Application Default Credentials; no keys are stored.
      </Step>
    </Steps>
  </Tab>
...
- **Grafana Mimir is licensed under AGPLv3.** Review the license if you plan to modify and redistribute it.
