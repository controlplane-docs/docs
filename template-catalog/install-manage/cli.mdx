...
<Accordion title="Required permissions">
  You need [policies](/reference/policy) to create the resources defined in the template, plus `reveal` permission on secrets to manage release state.
</Accordion>
</AccordionGroup>
...
<Steps>
  ...
  <Step title="Configure Values">
    ...
    ```yaml values.yaml
    # Example values — refer to the specific template's documentation for available options
    ```
  </Step>
  <Step title="Install the Release">
    ...
    {/* TODO: Confirm the exact --repo URL and whether --gvc is passed as a flag or set in values */}
