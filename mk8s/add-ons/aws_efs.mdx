---
title: AWS EFS
description: Enable Kubernetes workloads on Managed Kubernetes clusters to access Amazon EFS for shared, persistent file storage.
keywords: ['mk8s', 'nfs', 'shared volume', 'rwx', 'csi driver', 'pvc', 'pv', 'elastic file system', 'shared filesystem']
---
...
- **Using the Console**: If you're creating the cluster through the console, navigate to `Add-ons` in the left pane, find the `AWS EFS` add-on in the list of available add-ons, toggle it on, and then enter the ROLE ARN required for accessing the AWS EFS filesystem.
...
3. **Activate the Dashboard**: Choose `Add-ons` in the left pane, find the `AWS EFS` add-on in the list, and toggle it on.
