...
description: Build a P2P network across cluster nodes to speed up container image pulls by sharing cached layers between nodes.
keywords: ['mk8s', 'image cache', 'pull cache', 'spegel', 'image distribution', 'fast pull', 'docker pull', 'pull throttling']
---
...
- **Using the Console**: If you're creating the cluster through the console, navigate to `Add-ons` in the left pane, find the `Registry Mirror` add-on in the list of available add-ons, and toggle it on.
...
3. **Enable Registry Mirror**: Choose `Add-ons` in the left pane and locate the `Registry Mirror` add-on from the list of available add-ons, then toggle it on.
