---
title: AWS ELB
description: Configure the AWS Load Balancer Controller for routing external traffic to your cluster using NLB, ALB, or CloudFront WAF integration.
keywords: ['ingress', 'lb', 'service type loadbalancer', 'mk8s', 'l4', 'l7', 'tcp', 'udp', 'aws-load-balancer-controller', 'tls passthrough']
---
...
3. **Enable the Add-on**: Choose `Add-ons` in the left pane, find the `AWS ELB` add-on in the list, and toggle it on.
4. **Optional: Enter Role ARN**: Select the AWS ELB menu  in the center pane. Provide the ROLE ARN necessary for AWS ELB Controller access to your AWS account.
