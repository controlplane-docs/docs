---
title: Logs
description: Send pod logs and Kubernetes audit records from your Managed Kubernetes cluster to Control Plane for centralized querying in the web UI or Grafana.
keywords: ['mk8s', 'log shipping', 'log forwarding', 'cluster logs', 'k8s audit', 'fluentd', 'fluentbit', 'loki', 'log aggregation']
---
...
- **Using the Console**: If you're creating the cluster through the console, navigate to `Add-ons` in the left pane, find the `Logs` add-on in the list of available add-ons, and toggle it on.
...
3. **Enable Logs**: Choose `Add-ons` in the left pane and locate the `Logs` add-on from the list of available add-ons, then toggle it on.
...
<Frame>
  <img src="/images/mk8s/add-ons/logs-configurations.png" alt="Logs add-on configuration UI showing namespace inclusion/exclusion filters and audit log toggle" />
</Frame>
...
<Frame>
  <img src="/images/mk8s/add-ons/ui-logs.png" alt="Control Plane Console logs view with LogQL query bar and filtered log entries for a Kubernetes cluster" />
</Frame>

Click on `Explore On Grafana` and analyze logs in Grafana:

<Frame>
  <img src="/images/mk8s/add-ons/grafana-logs.png" alt="Grafana Explore view showing Kubernetes cluster logs with time-series graph and log detail panel" />
</Frame>
