...
description: Allow pods on Managed Kubernetes clusters to securely assume AWS IAM Roles for accessing AWS resources like S3, DynamoDB, and other services.
keywords: ['irsa', 'mk8s', 'pod identity', 'sts', 'service account', 'oidc provider', 'least privilege', 'cross account', 'iam role for service account']
---
...
- **Using the Console**: If you're creating the cluster through the console, navigate to `Add-ons` in the left pane, find the `AWS Workload Identity` add-on in the list of available add-ons, and toggle it on.
...
1. **Enable the AWS Workload Identity add-on**: Choose `Add-ons` in the left pane and locate the `AWS Workload Identity` add-on from the list of available add-ons, then toggle it on.
