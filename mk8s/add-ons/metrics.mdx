---
title: Metrics
description: Send Prometheus metrics from your Managed Kubernetes cluster to Control Plane for centralized monitoring in the web UI or Grafana.
keywords: ['mk8s', 'observability', 'time series', 'tsdb', 'cluster metrics', 'kube-state-metrics', 'node-exporter', 'remote write', 'mimir']
---
...
- **Using the Console**: If you're creating the cluster through the console, navigate to `Add-ons` in the left pane, find the `Metrics` add-on in the list of available add-ons, and toggle it on.
...
3. **Enable Metrics**: Choose `Add-ons` in the left pane and locate the `Metrics` add-on from the list of available add-ons, then toggle it on.
