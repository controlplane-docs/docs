...
- The [AWS Workload Identity](/aws_workload_identity) add-on must be enabled for your cluster.
...
You can enable the AWS ECR add-on for your Kubernetes cluster either during the cluster creation process or at any time afterward.
The following sections outline the methods for enabling the add-on:

### [At Cluster Creation](#at-cluster-creation)

#### [AWS Providers Accessing Private ECR in the Same Account](#accessing-aws-same-account)
...
#### [For NON-AWS Providers or Different AWS Account Access](#accessing-non-aws)
...
#### [AWS Providers Accessing Private ECR in the Same Account](#accessing-aws-same-account-cluster)
...
#### [For NON-AWS Providers or Different AWS Account Access](#accessing-non-aws-cluster)
...
### [Accessing ECR in Same AWS Account](#usage-same-account)

If you are using an AWS provider to access ECR within the same AWS account, follow these steps:

#### [Step 1 - Create OIDC Identity Provider in AWS (If not already done)](#create-oidc-provider-same)
...
#### [Step 2 - Creating Kubernetes workload](#create-kubernetes-workload-same)
...
### [Accessing ECR with NON-AWS Provider or Different AWS Account](#usage-same-account-no)

If you are using a non-AWS provider or an AWS provider to access ECR in a different AWS account, follow these steps:

#### [Step 1 - Create OIDC Identity Provider in AWS (If not already done)](#create-oidc-provider-no)
...
#### [Step 2 - Update Trust Policy](#update-trust-policy-no)
...
#### [Step 3 - Creating Kubernetes workload](#create-kubernetes-workload-no)
