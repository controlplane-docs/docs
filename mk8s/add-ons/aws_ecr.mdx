---
title: AWS ECR
description: Enable Managed Kubernetes clusters to pull container images from private Amazon ECR registries.
keywords: ['image pull', 'image pull secret', 'private registry', 'mk8s', 'iam role', 'cross account', 'ecr login', 'docker registry']
---
...
- **Using the Console**: If you're creating the cluster through the console, navigate to `Add-ons` in the left pane, find the `AWS ECR` add-on in the list of available add-ons, and toggle it on.
...
- **Using the Console**: If you're creating the cluster through the console, navigate to `Add-ons` in the left pane, find the `AWS ECR` add-on in the list of available add-ons, toggle it on, and then enter the ROLE ARN required for accessing the AWS ECR repository.
