...
description: Enable the Rancher Local Path Provisioner to automatically create persistent volumes backed by hostPath or local storage on cluster nodes.
keywords: ['mk8s', 'pvc', 'pv', 'k8s storage', 'local volume', 'csi', 'rancher.io', 'storageclass', 'ephemeral', 'node disk']
---
...
- **Using the Console**: If you're creating the cluster through the console, navigate to `Add-ons` in the left pane, find the `Local Path Storage` add-on in the list of available add-ons, and toggle it on.
...
3. **Activate the Dashboard**: Choose `Add-ons` in the left pane, find the `Local Path Storage` add-on in the list, and toggle it on.
