---
title: GCP Provider
description: Deploy Managed Kubernetes worker nodes in your GCP project. Requires only an existing network and subnet as prerequisites.
keywords: ['mk8s', 'gke alternative', 'google cloud', 'compute engine', 'gce', 'vpc', 'service account', 'spot vm', 'gke']
---
...
2. **Create the Cluster**: Deploy the `gcp-mk8s-example` cluster by applying the manifest.
