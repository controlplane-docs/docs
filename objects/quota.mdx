---
title: 'Quota'
---

## Overview

The quotas page will display the currently used and allowed allocation of Control Plane resources for your [org](/objects/org).

Current resources with a quota:

| Resource              | Description                          |
| :-------------------- | :----------------------------------- |
| gvcs                  | Max number of gvcs                   |
| agents                | Max number of agents                 |
| secrets               | Max number of secrets                |
| domains               | Max number of domains                |
| cloudaccounts         | Max number of cloud accounts         |
| serviceaccounts       | Max number of service accounts       |
| ipsets                | Max number of ip sets                |
| volumesets-per-gvc    | Max number of volume sets in a gvc   |
| identities-per-gvc    | Max number of identities in a gvc    |
| workloads-per-gvc     | Max number of workloads in a gvc     |
| cpu-per-workload      | Max cpu millicores of a workload     |
| memory-per-workload   | Max memory (MiB) of a workload       |
| replicas-per-workload | Max number of replicas of a workload |

If your org requires additional resources, please email [support@controlplane.com](mailto:support@controlplane.com)

## Permissions

The permissions below are used to define [policies](/objects/policy) together with one or more of the four
[principal types](/platform/access-control):

| Permission | Description            | Implies                    |
| :--------- | :--------------------- | :------------------------- |
| create     | Create new quotas      |                            |
| edit       | Modify existing quotas | view                       |
| manage     | Full access            | create, edit, manage, view |
| view       | Read-only access       |                            |

## API Reference

Click to see [API Agent Reference](/api-reference/agent)

## CLI Reference

Click to see [CLI Agent Reference](/references/cli/agent)

## Terraform Reference

Click to see [Terraform Agent Resource](/references/terraform/resources/cpln_gvc)

Click to see [Terraform Agent Data Source](/references/terraform/data-sources/cpln_gvc)
