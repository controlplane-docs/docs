...
A volume set is a collection of storage volumes, which are used by a stateful workload. A volume set provides a unique volume for each replica in the linked workload. Each volume set can be used by at most one [stateful workload](/objects/workload/overview#types).
...
<Warning>Shared file system support is currently in beta.</Warning>

- Control Plane provisions a single volume per location
- The volume set can be attached to any number of workloads - All attached workload replicas in a given location use the same volume
...
<Note>
  In general, IOPS/throughput capacity varies linearly with storage capacity. The values shown below are the maximum possible values, and
  may only be achievable with large volume sizes.
</Note>
