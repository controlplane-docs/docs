...
Secrets are used to store strongly-encrypted information. All secrets can be injected into containers through the use of [environment variables](/common-configs/environment-variables).

In addition to optionally being injected into containers as environment variables, secrets are also used for:

1. Image registry ([Docker](#docker), [ECR](#ecr), and [GCP](#google-cloud-platform-gcp)) pull secrets, enabling workloads in a [GVC](/objects/gvc) to pull images from private repositories.
...
A [workload identity](/objects/identity) **MUST** be granted the [reveal permission](#reveal-permission) on a secret to have its value injected at runtime.
...
The AWS secret type enables a [workload](/objects/workload) to consume services from AWS. It is used in cases where an [identity](/objects/identity) isn't used.

For example, when an Org does not wish to create a [cloud account](/objects/cloudaccount), or to create cloud access policy as part of an identity definition.
...
- Used to store [Azure SDK](/objects/cloudaccount#azure-details) credentials.
- When [creating a Cloud Account using the Azure SDK](/guides/cloudaccount/connecting-to-azure-services-with-azure-sdk), a JSON file is generated containing the credentials. This file is uploaded and automatically stored as an Azure-SDK secret.
...
- Used to store [Azure Function App](/objects/cloudaccount#azure-details) credentials.
- When [creating a Cloud Account using the Azure Connector](/guides/cloudaccount/connecting-to-azure-services-with-azure-connector), the `deployment URL` and `code` will be saved.
- These values can be updated using the console or the [CLI](/reference/cli#secret-update).

### Docker

The Docker secret type is used when a [container](/objects/workload/containers) pulls an [image](/objects/workload/containers#images) from a private Docker compatible repository, such as a Org's private repository at Control Plane, [Docker Hub](https://hub.docker.com/), or [Azure Container Registry](https://docs.microsoft.com/en-us/azure/container-registry/).

An authorized user then assigns a secret of this type to a [GVC Pull Secret](/objects/gvc#pull-secrets).
...
To create a Docker secret targeting a private repository hosted at Control Plane, a [Service Account](/objects/serviceaccount) at the source Org, bound with the [pull permission](/objects/image#pull) is required.
...
- Used to securely store key/value pairs that can be used by a [workload's environment variables](/common-configs/environment-variables).
...
- Similar to a [Docker](#docker) and [ECR](#ecr) secret, a GCP secret is used when a workload pulls a container image from a private `Google Container Registry`.
...
- Used to securely store a keypair that can be used by a [workload's environment variables](/common-configs/environment-variables).
...
To access the stored text, an [environment variables'](/common-configs/environment-variables) value must be set to one of the following:
...
- When creating a [Cloud Account using the Azure Connector](/objects/cloudaccount#azure-connector), the `deployment URL` and `code` will be saved.
- These values can be updated using the console or the [CLI](/reference/cli#secret-update).
...
- Used to securely store a Transport Layer Security (TLS) secret that can be used by a [workload's environment variables](/common-configs/environment-variables).
...
- Used to securely store a username and password secret that can be used by a [workload's environment variables](/common-configs/environment-variables).
...
<Note>Each `reveal` action is captured in the audit trail of built-in `cpln` audit context.</Note>
...
- Policy's [binding](/objects/policy#bindings)
