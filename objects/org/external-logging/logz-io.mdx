...
Logz.io requires an authorization token in order to process data and logs. You’ll need to store the token as an [Opaque Secret](/objects/secret#opaque) and configure external logging.
...
4. Select the Opaque secret created to authenticate to Logz.io. Refer to the [generate token credentials](#step-1-generate-token-credentials) section to obtain and configure the necessary credentials.
5. Click Save.
