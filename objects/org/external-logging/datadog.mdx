...
Follow these steps to obtain the API key, store it as an [Opaque Secret](/objects/secret#opaque), and configure external logging.
...
4. Select the Opaque secret created to authenticate to Datadog. Refer to the [credential procurement](#step-1-credential-procurement) section to obtain and configure the necessary credentials.
5. Click `Save`.
