...
Cloud Object and File storage, ephemeral scratch storage and [Secrets](/objects/secret) can be mounted to directories of [containers](/objects/workload/containers) at runtime by adding one or more volumes.
...
During the set up of a volume using the console, the `uri` name can be entered manually or an existing [Cloud Account](/objects/cloudaccount) can assist looking up the name.

The [identity](#identity-configuration) of the workload is used to authenticate to the provider's cloud storage API, or used for authorization to access the Control Plane secret. A [Cloud Account](/objects/cloudaccount) for each cloud storage provider, with the necessary access/roles, must exist and be associated with
...
| :------------------------------------ | :--------------- | :-------------------- | :------------------------------------- |
| [CPLN Secret](/objects/secret)        | cpln://secret    | read-only             | cpln://secret/secretname               |
| CPLN Secret (With Property Reference) | cpln://secret    | read-only             | cpln://secret/mydictionary.keyname     |
| [CPLN Volume Set](/objects/volumeset) | cpln://volumeset | read-write            | cpln://volumeset/my-volume-set         |
...
- [Opaque Secret](/objects/secret#opaque)
  :
...
- [Azure](/objects/secret#azure-sdk), [Docker](/objects/secret#docker), and [GCP](/objects/secret#google-cloud-platform-gcp) Secrets:{' '}
...
- All other [Secret Types](/objects/secret#secret-types):
...
A Workload that is configured with a volume that references a [Secret](/objects/secret) must be configured with an [Identity](/objects/identity) bound to a [policy](/objects/policy) having the [reveal](/objects/secret#reveal-permission) permission.

To allow a workload identity the ability to authenticate to an object store, a [cloud access](/objects/identity#cloud-access-universal-cloud-identity) rule must be created for each provider. A [Cloud Account](/objects/cloudaccount) for each provider must exists in order to create the cloud access rule.
...
To allow a Workload access to the object stores, the outbound requests of its [external firewall](/objects/workload/firewall#external) must either be set to `All Outbound Requests Allowed` or the hostnames listed below for the corresponding object store must be added to the `Outbound Hostname Allow List`.
