...
Workloads run in Control Plane’s AWS, Azure, GCP accounts or in your own [Custom Location](/objects/location#byok-locations) (BYOK), where clouds/regions are determined by the [GVC](/objects/gvc) definition. Your workload may run only in a single region of one cloud, or across many regions of all the three clouds – completely up to the GVC definition. Requests are routed to the nearest healthy location.
...
- **Stateful**:
  - Simlilar to a `standard` workload, `stateful` workloads have stable replica identities and hostnames, and can mount a [volume set](/objects/volumeset) for persistent storage.
...
See [Autoscaling](/objects/workload/autoscaling) for more information.
...
By default, both [Capacity AI](#capacity-ai) and [Auto Scaling](#auto-scaling) settings are applied to all deployments at each location enabled in the [GVC](/objects/gvc). However, these settings can be customized at each location to enhance performance for specific audiences.
...
<Note>When configuring probes, it is not recommended to set is as TCP, because it is not as reliable as other methods.</Note>

## Alerts

Using Grafana, you can create alerts on any of the standard metrics exposed by Control Plane, or on your [custom metrics](/objects/workload/custom-metrics). To access Grafana, navigate to one of your orgs in the Control Plane console and click the "Metrics" link.
...
When an identity is not set for a workload, it is assigned a default identity which is only able to get itself from the platform.
