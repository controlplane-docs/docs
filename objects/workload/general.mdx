...
<Note>
  When a gvc has multiple locations, and it has multiple workloads which suspends all locations except one, the internal communication will always try the same location. So if the same location is suspended but another location is available, they won't be able to communicate.
</Note>
