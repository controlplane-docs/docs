...
Refer to the [Workload overview](/objects/workload/overview) page.

## Create a Workload

Refer to the [Create a Workload](/guides/create-resources/create-workload) guide for additional details.
...
Refer to the [identities](/objects/identity) page for additional details.
...
<Note>
  When a gvc has multiple locations, and it has multiple workloads which suspends all locations except one, the internal communication will
  always try the same location. So if the same location is suspended but another location is available, they won't be able to communicate.
</Note>
...
| CPLN_GVC              | The Global Virtual Cloud the container is running under                                                                                                     | string                                           |
...
Refer to the [logs](/platform/logs) page for additional details.
...
Example: If there are 4 running replicas and maxSurgeReplicas is set to 50%, then during each rollout 2 replicas will be added at the new version. Once they are healthy as determined by the [ReadinessProbe](/objects/workload/containers#readiness-probe), the rollout will continue, -2 old replicas, +2 new replicas, -2 old replicas.
...
Workload containers can allocate GPU resources but it will require a different set of minimums for CPU and Memory resources, depending on the GPU type and count.
...
Workload health is a computed value for ease-of-use purposes. Atomic value of health is whether a container, version and location is reporting ready or not under deployments.
