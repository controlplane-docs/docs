...
[Workloads](/objects/workload) managed at Control Plane can be configured to serve requests using any domain name you own.

Domains are scoped to an [Org](/objects/org) and are associated with **one** [GVC](/objects/gvc). The domain can then be configured to handle requests for one or multiple Workloads within that GVC.

Domains that are configured at Control Plane are automatically secured using TLS certificates, load balanced, and DNS geo-routed to the nearest healthy [location](/objects/location) assigned to the GVC.
...
<Note>
  Current method of mapping a domain to a GVC is with [Subdomain Based Routing](#subdomain-based-routing).

Assigning a domain directly under gvc item is `legacy` and shouldn"t be used for new configurations.

</Note>

- Canonical endpoints (Global): `cpln.app`
  Follows the pattern of `{workloadName}-{gvcAlias}.cpln.app`
- Individual location endpoints: `controlplane.us`
  Follows the pattern of `{workloadName}-{gvcAlias}.{location}.controlplane.us`
...
port configured on the container. See table below

</Note>

| Domain Port Protocol | Compatible Container Port Protocols |
| :------------------- | :---------------------------------- |
| HTTP2                | HTTP2 GRPC                          |
| HTTP                 | HTTP                                |
| TCP                  | TCP                                 |
...
The certificate authority PEM, stored as a [TLS Secret](/objects/secret#tls), can be associated with the Domain and used to verify the authority of the client certificate. The only verification performed checks that the CN of the PEM matches the Domain (i.e., **CN=\*.DOMAIN**).
...
See the [GVC dedicated load balancer](/objects/gvc#dedicated-load-balancer) page for additional details.
