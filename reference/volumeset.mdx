...
A volume set is a collection of storage volumes, which are used by a stateful workload. A volume set provides a unique volume for each replica in the linked workload. Each volume set can be used by at most one [stateful workload](workload#stateful).
...
The `spec.autoscaling` object allows you to specify rules for vertically scaling volumes in the set. The available options are:
