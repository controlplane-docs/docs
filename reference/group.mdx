---
title: Group
description: "Groups organize users and service accounts for access control. Supports direct membership assignment and dynamic query-based membership rules."
keywords: ['principal', 'role group', 'cpln group', 'membership query', 'tag-based group', 'group binding', 'team', 'admin group', 'rbac']
---
...
| :------------ | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `-allmembers` | Every user, group, and service account in the org. It is added to the principal list on every authenticated request, so a binding on it reaches everyone. The [built-in policy](/reference/policy#built-in-policies) on the `org` kind uses it to grant all members `view`. |
| `-anonymous`  | Unauthenticated callers. It is added to the principal list on every request, authenticated or not, so a binding on it reaches everyone.                                                                                                                                                                                                                                            |
