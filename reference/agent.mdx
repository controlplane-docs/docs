...
Agents run in active-active mode. If an agent misses a set amount of heartbeats, it is considered offline and the remaining active agents continue handling work until the instance group automatically replaces it. 
...
To view the CLI documentation for an agent, [click here](/cli-reference/commands/agent).
