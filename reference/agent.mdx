...
## Protocol Version
Control Plane updated agents in March 2026, to support new features and enhance performance. 
...
2. The protocol version may be populated in the `status.protocolVersion` field. If the field is not present, the agent is running protocol version `v1`. If the field is present and has a value of `v2`, the agent is running protocol version `v2`.
