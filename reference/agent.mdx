---
title: Agent
description: "Wormhole agents for secure connectivity between Control Plane workloads and private networks. Covers high availability, sizing, agent types, and setup."
keywords: ['cpln agent', 'tunnel', 'vpn agent', 'reverse tunnel', 'on-prem connector', 'vpc peering', 'private endpoint', 'hybrid networking', 'self-hosted agent', 'install agent']
---
...
<Warning>
  `v1` is  deprecated. Please refer to the updated [Configure an Agent](/guides/setup-agent) guide, unless you created you argent before April 2026
</Warning>
...
- **AWS**
  ...
  Available Baseline Bandwidth for each instance can be discovered using the AWS CLI.
