...
In situations where your [workload](workload) needs to consume services from endpoints within a VPC or another private network, you would use a wormhole **agent**.

Control Plane's wormhole technology securely connects your [workloads](workload) to any TCP or UDP endpoints in VPCs and other private networks, including on-prem data centers and even on a developer's laptop. You establish a wormhole by running an agent VM (agent for short) inside the private network to which you are connecting your [workloads](workload).

Control Plane offers agents for both private networks and cloud providers. The agent is installed and configured within the location where it can access the applications or services that your [workload](workload) requires. It does this by establishing a secure and persistent connection to publicly hosted Control Plane servers. Requests from your workload are tunneled to the agent which performs the request and then tunnels the response back. This flow occurs in a performant manner and is transparent to the [workload](workload).

Agents are scoped to an [org](org) and are used in conjunction with [identities](identity) to set up [network resources](identity#network-resources-cloud-wormhole).
...
- [Amazon Web Services (AWS)](/guides/configure-resources/setup-agent#aws)
- [Microsoft Azure](/guides/configure-resources/setup-agent#azure)
- [Google Cloud Platform (GCP)](/guides/configure-resources/setup-agent#gcp)
- [Private Network](/guides/configure-resources/setup-agent#private-network) (e.g., private data centers, internal web services, developer's laptop, etc.)
- [Kubernetes (k8s) Cluster](/guides/configure-resources/setup-agent#kubernetes-k8s-cluster)
