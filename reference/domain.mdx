...
For **stateful** workloads, when the target workload has replicaDirect enabled, each route can specify a replica number to route traffic directly to a specific replica. The replica number must be an integer greater than or equal to 0.

If no replica is specified, traffic will be routed to all replicas of the workload.

#### Header Operations

Routes can be configured to manipulate HTTP headers for all requests to that route. Header operations allow you to set or override headers before forwarding requests to the workload.
...
- `http01`: Uses HTTP-01 challenge for certificate validation
- `dns01`: Uses DNS-01 challenge for certificate validation
...
**Certificate Challenge Types:**
- `dns01`: Uses DNS-01 challenge for certificate validation (only supported option)

<Note>HTTP-01 challenge type is not supported for domains with NS mode.</Note>
...
The default TLS protocol version is **TLSV1_2**. The minimum TLS protocol version is **TLSV1_0**.
...
The certificate authority PEM, stored as a [Secret](/reference/secret) of type `keypair`, can be associated with the Domain and used to verify the authority of the client certificate. The secret must contain a client certificate authority cert in PEM format used to verify requests which include client certificates. The key subject must match the domain and the key usage properties must be configured for client certificate authorization.
...
A Custom Server Certificate can be assigned to a Domain by selecting an existing [Secret](/reference/secret) of type `keypair`.
...
1. [Accept All Subdomains](#accept-all-subdomains)
1. [Host Prefix](#host-prefix)
1. [Host Regex](#host-regex)


#### Accept All Hosts

`Accept All Hosts` configures the domain to allow all traffic (i.e. wildcard support) to the configured workloads, regardless of what the Host header or SNI is for the request. There should only ever be at most one Domain configured with this setting for a single GVC. If there are multiple, then requests will only route for one of the two Domains.

#### Accept All Subdomains

`Accept All Subdomains` configures the domain to allow all traffic to `*.${Domain}` for the domain. No other Domains in the same GVC should be configured for any subdomains of this Domain since their routing rules will be ignored.

#### Host Prefix

For domains using [Path Based Routing](#path-based-routing) the `Host Prefix` will be enabled and can be used for each path when the domain has the `Accept All Hosts` or `Accept All Subdomains` property enabled.
...
For domains using [Path Based Routing](#path-based-routing) the `Host Regex` will be enabled and can be used for each path when the domain has the `Accept All Hosts` or `Accept All Subdomains` property enabled.
...
- `hostPrefix` and `hostRegex` can only be used when wildcard support is enabled
