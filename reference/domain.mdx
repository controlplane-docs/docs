...
Domains are scoped to an [org](/reference/org) and are associated with **one** [Global Virtual Cloud (GVC)](/reference/gvc). The domain can then be configured to handle requests for one or multiple [Workloads](/concepts/workload) within that [GVC](/reference/gvc).
