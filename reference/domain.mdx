...
[Workloads](workload) managed at Control Plane can be configured to serve requests using any domain name you own.

Domains are scoped to an [Org](org) and are associated with **one** [GVC](gvc). The domain can then be configured to handle requests for one or multiple [Workloads](workload) within that [GVC](gvc).

Domains that are configured at Control Plane are automatically secured using TLS certificates, load balanced, and DNS geo-routed to the nearest healthy [location](location) assigned to the [GVC](gvc).

## Default Domain Names

If a Domain is not mapped to a [GVC](gvc), the default domain names provided to [Workloads](workload) are:
...
Refer to the [Configure a Domain](/guides/configure-resources/configure-domain) guide for additional details.
...
Once the apex domain is verified, subdomains can be added to the same [Org](org) without needing the TXT record verification step.

If multiple [Orgs](org) are creating subdomains using the same apex domain, the apex domain verification only needs to be performed in one of the Orgs, but subdomains in Orgs that don't have the apex domain will require a TXT record be added to DNS for verification.
...
Subdomain based routing allows a domain to be mapped to all workloads in a [GVC](gvc).
...
The certificate authority PEM, stored as a [TLS Secret](secret#tls), can be associated with the Domain and used to verify the authority of the client certificate. The only verification performed checks that the CN of the PEM matches the Domain (i.e., **CN=\*.DOMAIN**).

If a certificate authority PEM is not associated with a Domain, no verification is performed.

CRL lists are not verified / checked, but they can be checked by the [Workload](workload) by keeping a list of allowed or revoked client certificate hashes. When a request is received by the [Workload](workload), the hash field in the XFCC header can be checked against the allowed or revoked list and an allow / deny decision can be made.
...
See the [GVC dedicated load balancer](gvc#dedicated-load-balancer) reference page for additional details.
