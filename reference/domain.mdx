...
Using the Console UI, the port configuration section can be accessed when editing a domain and using `Advanced Mode`.
...
Refer to the [dedicated load balancer](#dedicated-load-balancer-options) section for which external ports can be used if dedicated load balancing is enabled.
