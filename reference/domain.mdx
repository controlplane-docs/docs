...
keywords: ['apex domain', 'subdomain', 'dns01', 'http01', 'fqdn', 'host header', 'wildcard cert', 'path route', 'subdomain route', 'cors', 'custom cert', 'mtls', 'client certificate', 'client ip header', 'private domain', 'certificate generator', 'acme']
---
...
spec:
  dnsMode: cname # or "ns"
  certChallengeType: http01 # or "dns01"; omit when certificateGeneratorLink is set
  # certificateGeneratorLink: //certificategenerator/example # optional, see Certificate Generators
  gvcLink: //gvc/my-gvc
  ports:
    - number: 443
      protocol: http2
...
- `certificateGeneratorLink` with a `dns01` [certificate generator](/reference/certificategenerator): Control Plane issues the wildcard certificate through your own CA and solves the challenge in your DNS zone. No `_acme-challenge` delegation and no custom server certificate are needed.
...
`certChallengeType` applies to the built-in Let's Encrypt flow only. It must be omitted when `certificateGeneratorLink` is set.

## Certificate Generators

By default, certificates are issued by Let's Encrypt and challenges are solved by Control Plane. To issue certificates through a different ACME certificate authority, or to solve DNS challenges with your own Route53, Cloud DNS, Azure DNS or Cloudflare credentials, create a [certificate generator](/reference/certificategenerator) and reference it from the domain:
...
The generator defines the challenge type, so `certChallengeType` must not be set, and `tls.serverCertificate` cannot be combined with it. An HTTP-01 generator only issues certificates for the exact domain name and is therefore rejected for NS mode domains, domains with `workloadLink`, and CNAME domains with `gvcLink`. Not supported for [internal domains](#internal-domains). Setting or changing the link requires the `use` permission on the generator.
