...
keywords: ['domain','dns','routing','workload','gvc','tls','certificate','cname','ns','geo-routing']
---

## Domain Overview
[Workloads](/concepts/workload) managed at Control Plane can be configured to serve requests using domain that you own.
...
Path based routing allows requests matching a specific path prefix to be routed to a specific workload. Multiple paths can be defined, but there must be at least one path defined.
...
The following CORS properties can be configured on the domain. Depending on the application being served, these setting could be configured from the application.
...
This ensures that your Domains are remain secured with up-to-date certificates without requiring manual intervention.
