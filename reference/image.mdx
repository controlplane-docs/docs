...
Control Plane offers its users their own hosted private image registry. Your application will be packaged and stored as an image and will be served by a [Workload](workload).

When configuring the source of the [Workload's](workload) container image, you have the option of using either an external private/public image registries, such as [Docker Hub](https://hub.docker.com/), or the internal private image registry offered by the platform.

The [CLI](cli#image-build) contains the functionality to build and push your application to your private image registry.
...
Refer to the [Pull an Image](/guides/image-management/pull-image) guide for additional details.

## Push an Image

Refer to the [Push an Image](/guides/image-management/push-image) guide for additional details.
...
Unless the [policy](policy) targets all images, a [query](cli#query) must be created with the image names (without the tag) that the principal is allowed to pull. That [query](cli#query) uses the `property` parameter and can only be created/updated using [cpln apply](/guides/cli/cpln-apply) or the CLI's [cpln profile](cli#policy) command.
