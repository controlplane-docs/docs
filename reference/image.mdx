...
When configuring the source of the [Workload's](/concepts/workload) container image, you have the option of using either an external private/public image registry, such as [Docker Hub](https://hub.docker.com/), or the internal private image registry offered by the platform.

The [CLI](/cli-reference/commands/image#image-build) contains the functionality to build and push your application to your private image registry.
...
Refer to the [Pull an Image](/cli-reference/guides/pull-image) guide for additional details.

## Push an Image

Refer to the [Push an Image](/cli-reference/guides/push-image) guide for additional details.

<Note>

As of `cpln` CLI `v3.7.2`, `cpln image build` executes `docker buildx build`. If Buildx is missing from the Docker CLI, the command fails with errors like `unknown shorthand flag: 'f' in -f`. Follow Docker's [Buildx installation guide](https://docs.docker.com/build/install-buildx/) or the steps in the [Push an Image guide](/cli-reference/guides/push-image#enable-docker-buildx-cli-372) to install/enable the plugin (especially on CI runners) before building via the CLI.

</Note>

## Copy an image

Using the [CLI](/cli-reference/commands/image#image-copy), to copy an image from one org to another, execute the following command:
...
Using the [CLI](/cli-reference/overview), to update the image a container is pulling, execute the following command:
...
To view the CLI documentation for images, [click here](/cli-reference/commands/image).
