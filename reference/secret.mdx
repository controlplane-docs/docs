---
title: Secret
description: "Encrypted secret storage with 12 types: AWS, Azure SDK, Azure Connector, Docker, ECR, GCP, Dictionary, Keypair, Opaque, NATS Account, TLS, and Username & Password."
keywords: ['credentials', 'env var injection', 'pull secret', 'cpln secret', 'vault', 'kms', 'secret manager', 'private key', 'cert secret', 'api key storage', 'database password', 'oauth token']
---
...
1. Image registry ([Docker](#docker), [ECR](#ecr), and [GCP](#google-cloud-platform-gcp)) pull secrets, enabling workloads in a [Global Virtual Cloud (GVC)](/reference/gvc) to pull images from private repositories.
