...
Secrets are used to store strongly-encrypted information. All secrets can be injected into containers through the use of [environment variables](workload#environment-variables).

In addition to optionally being injected into containers as [environment variables](workload#environment-variables), secrets are also used for:

1. Image registry ([Docker](#docker), [ECR](#ecr), and [GCP](#google-cloud-platform-gcp)) pull secrets, enabling workloads in a [GVC](gvc) to pull images from private repositories.
...
A [workload identity](identity) **MUST** be granted the [reveal permission](#reveal-permission) on a secret to have its value injected at runtime.
...
The AWS secret type enables a [workload](workload) to consume services from AWS. It is used in cases where an [identity](identity) isn't used.

For example, when an Org does not wish to create a [cloud account](cloudaccount), or to create cloud access policy as part of an [identity](identity) definition.

The recommended approach to consuming services from AWS is not to use a AWS secret, but rather to:

1. Create a [cloud account](cloudaccount).
2. Create an [identity](identity).
3. Defining a cloud access policy for the AWS account as part of the identity.

In cases where an Org wishes to not leverage the [identity](identity) feature, in order to use an AWS secret, simply read the secret from the environment variable and convey its set of values to AWS using any AWS API you wish.
...
- When creating a [Cloud Account using the Azure SDK](cloudaccount#azure-sdk), a JSON file is generated containing the credentials. This file is uploaded and automatically stored as an Azure-SDK secret.
...
The Docker secret type is used when a [container](workload#containers) pulls an [image](workload#images) from a private Docker compatible repository, such as a Org's private repository at Control Plane, [Docker Hub](https://hub.docker.com/), or [Azure Container Registry](https://docs.microsoft.com/en-us/azure/container-registry/).

An authorized user then assigns a secret of this type to a [GVC Pull Secret](gvc#pull-secrets).

[Workloads](workload) in the [GVC](gvc) can then pull their image using the secret.
...
To create a Docker secret targeting a private repository hosted at Control Plane, a [Service Account](serviceaccount) at the source Org, bound with the [pull permission](image#pull) is required.
...
This secret type is used when configuring a [GVC's Pull Secret](gvc#pull-secrets). [Workloads](workload) that are connected to that [GVC](gvc) will have access to the secret when pulling the image.
...
- Similar to a [Docker](#docker) and [ECR](#ecr) secret, a GCP secret is used when a [workload](workload) pulls a container image from a private `Google Container Registry`.
- This secret type is used when configuring a [GVC Pull Secret](gvc#pull-secrets). [Workloads](workload) that are connected to that [GVC](gvc) will have access to the secret when pulling the image.
...
The plaintext of a secret can be viewed only if you or an [identity](identity) has the `reveal` permission.

For example, when using a secret as the value of an [environment variable](workload#environment-variables) for a container, the [identity](identity) assigned to the [workload](workload) must have the `reveal` permission set on the assigned secret using a [policy](policy).

When configuring the [environment variable](workload#environment-variables), the value of the variable will be in the following format: `cpln://secret/SECRET_NAME`. The image running within the container will be able to access the plaintext of the secret by referring to the name of the configured [environment variable](workload#environment-variables).
