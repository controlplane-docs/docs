...
Pull secrets are [secrets](/reference/secret) assigned to a GVC and used by its [workloads](/concepts/workload) to authenticate to a private registry. Set them in `spec.pullSecretLinks`.

Only the [Docker](/reference/secret#docker), [Amazon ECR](/reference/secret#ecr), and [GCP](/reference/secret#google-cloud-platform-gcp) secret types qualify.

<Tip>

An image pushed to the Control Plane registry of the same [org](/reference/org) needs no pull secret.

</Tip>

A GVC can hold several pull secrets. A [workload's container](/reference/workload/containers) uses whichever one authenticates to the registry it is pulling from, trying each in turn.

If every secret fails, the deployment is not updated and image pulls retry with exponential backoff from 10 seconds up to 5 minutes (10 seconds, 20 seconds, 40 seconds, and so on).
...
- **`secrets`**: A list of [dictionary secrets](/reference/secret#dictionary) to be used as TriggerAuthentication objects. Each secret becomes a TriggerAuthentication object named after the secret, and each key of the dictionary becomes an authentication parameter with the same name. Workload triggers reference it by the secret name via `authenticationRef` — see [TriggerAuthentication](/reference/workload/autoscaling#triggerauthentication). No identity or secret reveal policy is required; the platform reveals these secrets on your behalf.
