...
A user is a member of one or more [orgs](org).

Users are granted specific access permissions to the various resources within the platform. These grants can be applied to a user by being a member of a [group](group) or [policy](policy).

## Invite Users

Refer to the [Invite Users](/guides/configure-resources/invite-users) guide for additional details on how to invite new users to your [org](org).

## Multiple Orgs

A user can be a member of one or more [orgs](org).

- To switch between [orgs](org) from the console:
...
To view the CLI documentation for users, [click here](cli#user).
