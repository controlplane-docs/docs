...
Each action that occurs on the Control Plane UI console or [CLI](cli) is reliably captured, securely stored, and indexed using the audit trail. When using the Control Plane UI console, most resources have an audit trail link that will redirect to the audit trail page with the resource ID prefilled. Additional filters can be added to drill-down to specific events.
...
The architecture of the audit trail is generic and allows any [workload](workload) to capture any action securely and reliably.

For your [workload](workload) to use the audit trail, it must first create an [audit context](auditctx). Refer to the [audit context](auditctx) reference page for additional details.
...
- The Control Plane Audit Trail UI:
  - Within the UI, you can select the [audit context](auditctx) and only view those actions.
