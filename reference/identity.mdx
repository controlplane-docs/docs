...
A [workload](workload) needing to consume cloud resources from one or more cloud providers (e.g., AWS, Azure, and GCP) without storing credentials or needing to communicate to endpoints within a private network - must be assigned an **identity**.
...
- Network traversal rules from [workloads](workload) into specific endpoints in private networks (e.g., a VPC). These rules connect an [agent](agent) in a private network to the Control Plane fabric allowing [workloads](workload) to selectively access TCP endpoints inside private networks where [agents](agent) are installed and running.

An **identity** is scoped to a [GVC](gvc) and can be assigned to multiple [workloads](workload) within the same [GVC](gvc) needing the same cloud resources and network access.
...
- Tunnel [workload](workload) network requests to specific TCP endpoints within VPCs or other private networks. (Tunneling network traffic from [workloads](workload) to specific TCP hosts and ports is facilitated using [agents](agent). This capability is referred to as “wormholes”.)

<Warning>

Identities are powerful Control Plane resources that can be granted any permission to your cloud environment. The ability to create identities should only be given to administrators by using a [policy](policy).

</Warning>

## Create an Identity

Refer to the [Create an Identity](/guides/create-resources/create-identity) guide for additional details.
