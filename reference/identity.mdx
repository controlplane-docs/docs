---
title: Identity
description: "Workload identities for credential-free cloud provider access via Universal Cloud Identity. Covers AWS, GCP, Azure and network resources."
keywords: ['irsa', 'pod identity', 'service account binding', 'iam role', 'sts', 'cloud wormhole', 'least privilege', 'identity kind', 'principal', 'workload sa']
---

## Overview

**Identity** (also known as a workload identity) is one of the four Control Plane [principal types](/concepts/access-control) (users, service accounts, groups, and identities).
...
An **identity** is scoped to a [Global Virtual Cloud (GVC)](/reference/gvc) and can be assigned to multiple [workloads](/concepts/workload) within the same GVC that needs the same cloud resources and network access. Identities cannot be shared across GVCs. Different GVCs can have identities with the same name; they are independent resources.
...
To view the CLI documentation for Identities, see the [Identity CLI reference](/cli-reference/commands/identity).
