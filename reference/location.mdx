...
Locations are scoped to an [org](org) and the [org](org) administrator can turn off/on the locations that are permitted when configuring a [GVC](gvc).

By default, all locations are enabled. If a location is disabled, any [workloads](workload) within that location will be removed. Further, if a location is enabled, all [workloads](workload) will be provisioned to that location.
