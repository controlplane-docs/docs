---
title: Overview
description: "Reference section navigation and resource hierarchy. Covers org-level resources, GVC structure, principals, infrastructure, and asset relationships."
keywords: ['kinds', 'api objects', 'data model', 'spec', 'resource hierarchy', 'org scoped', 'gvc scoped', 'manifest schema', 'config schema', 'resource links']
---
...
Control Plane resources are organized into a hierarchical structure where resources are scoped to provide isolation and governance:
...
<Card title="Org (Organization)" icon="building" href="/reference/org" horizontal>
Top-level administrative boundary for all platform resources. Configure organization-wide governance, observability, security integrations, and operational settings.
...
<CardGroup cols={2}>
  ...
  <Card title="Group" icon="users" href="/reference/group">
    Collections of users and service accounts. Supports dynamic membership via queries on tags, properties, or relations.
  </Card>
...
<CardGroup cols={2}>
  ...
  <Card title="Workload" icon="server" href="/reference/workload/general">
    Deploy containerized applications as standard, stateful, cron, or serverless workloads with autoscaling and load balancing.
  </Card>
  <Card title="Location" icon="/icons/location.svg" href="/reference/location">
    Geographic regions across AWS, GCP, Azure, and OCI, plus BYOK locations backed by your own clusters.
  </Card>
...
<AccordionGroup>
  ...
  Control Plane implements hierarchical role-based access control (RBAC) where **Policies** bind permissions to **Principals** (users, groups, service accounts, identities) for target resources. Permission inheritance follows hierarchical implication rules; for example, `manage` implicitly grants `create`, `edit`, and `delete`.
  </Accordion>
  <Accordion title="Multi-Cloud Deployment">
    A single **GVC** can deploy workloads across multiple **Locations** spanning AWS, GCP, and Azure regions simultaneously. Platform networking automatically routes traffic to healthy workload instances based on availability and geographic proximity.
  </Accordion>
  ...
  <Accordion title="Private Network Access">
    **Agents** establish secure connectivity between Control Plane and private infrastructure without requiring public network exposure. Combined with **Identities**, workloads can securely access databases, internal APIs, and services within private cloud networks and VPC environments.
  </Accordion>
</AccordionGroup>
