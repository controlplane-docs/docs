...
#### minCpu
When CapacityAI is enabled, `minCpu` defines the lowest possible CPU reservation for a container.
When using the stateful workload type, `minCpu` defines the resource reservation for a container or in Kubernetes parlance, the CPU `request`.
...
#### minMemory
When CapacityAI is enabled, `minMemory` defines the lowest possible memory reservation for a container.
When using the stateful workload type, `minMemory` defines the resource reservation for a container or in Kubernetes parlance, the memory `request`.
