---
title: Containers
description: "Container configuration reference covering image sources, resource limits (CPU/memory), ports, environment variables, liveness and readiness probes, and volumes."
keywords: ['container spec', 'image pull', 'resource requests', 'health check', 'startup probe', 'env from secret', 'volume mount', 'ecr', 'gcr', 'docker hub', 'oci', 'multi-container']
---

## Overview

For general information about containers, refer to the [Kubernetes containers](https://kubernetes.io/docs/concepts/containers/) concepts page.
...
Control Plane supports the use of dynamic tags. If an image is pushed and the tag is not updated (e.g., `latest`), the platform will redeploy the new image within 5 minutes.
...
Refer to the [Kubernetes probe documentation](https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/) for additional details.
...
- **Millicore**: Since a whole core might be too much for many applications, the platform allows you to specify CPU resources in millicores. One core is equivalent to 1,000 millicores. Specifying `500` (in the console) or `500m` (in the YAML manifest) would allocate half a core.
...
- **Mebibyte**: A unit of digital data that is equal to 1,048,576 bytes. Specifying `1` and selecting Mi (in the console) or `1Mi` (in the YAML manifest) would allocate 1 Mebibyte.
- **Gibibyte**: A unit of digital data that is equal to 1,073,741,824 bytes. Specifying `1` and selecting Gi (in the console) or `1Gi` (in the YAML manifest) would allocate 1 Gibibyte.
...
Selecting a GPU raises the container's minimum CPU and memory to match the card, and [Capacity AI](/reference/workload/capacity) cannot be enabled alongside it.

| Model | CPU | Memory | Quantity |
| :--- | :--- | :--- | :--- |
| `t4` | 2000m to 8000m per GPU | 7Gi to 31Gi per GPU | 1 to 4 |
| `a10g` | 6000m to 8000m | 8Gi to 62Gi | 1 |

A `t4` container asking for more than one GPU, or for more than 14Gi of memory, must reserve 4000m per GPU rather than 2000m. Requests below a minimum are rejected with `CPU must be at least 2000 millicores based on your GPU selection, please increase CPU.` and the matching memory message.

Control Plane bills reserved CPU and memory, and there is no separate per-GPU line item. Those minimums are how a card's cost is reflected, which is why they rise with the card and with the quantity.

#### Custom GPUs

`spec.containers[].gpu.custom` requests a non-NVIDIA accelerator by its Kubernetes resource name instead, with an optional runtime class:
...
| Field | Description |
| :--- | :--- |
| `resource` | Required. The extended resource the nodes advertise, such as `amd.com/gpu`. Up to 64 characters. |
| `runtimeClass` | Optional. The runtime class the pod runs under. Control Plane uses the value you supply rather than its own. |
| `quantity` | 1 to 8. Defaults to `1`. |

<Note>
`custom` takes effect only on [CPLN BYOK](/byok/overview) locations, where your own nodes advertise the resource and register the runtime class. On Control Plane's own regions the request is ignored, so use `nvidia` there. A container sets one of `nvidia` or `custom`, never both; supplying both is rejected with `The parameter spec.gpu must contain one of nvidia or custom and not both`.
</Note>
