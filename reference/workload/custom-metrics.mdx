...
description: Collect Prometheus-formatted custom metrics from workload containers and use them for autoscaling and monitoring.
keywords: ['/metrics endpoint', 'metric scrape', 'prom format', 'app metrics', 'scaling on metrics', 'business metrics', 'prometheus exporter', 'metric port']
---
...
1. In the UI, navigate to `Workloads` in the selected Global Virtual Cloud (GVC) and select the workload for which you want to scrape metrics.
...
<Frame>
  <img src="/images/reference/workload/workload-custom-metrics-config.png" alt="Workload with the Actions dropdown open showing the Export sub-menu with JSON, YAML, JSON Slim, YAML Slim, and Terraform options" />
</Frame>
...
For Terraform documentation, see the [Workload metrics Terraform resource documentation](https://registry.terraform.io/providers/controlplane-com/cpln/latest/docs/resources/workload#nestedblock--container--metrics).
...
If you wish to filter which metrics are collected from your workload, you can specify regex patterns in the `dropMetrics` field.
  ...
  containers:
    - name: my-container
      metrics:
        ...
        port: 9090
        dropMetrics:
          - '^go_.*' # Drop all go runtime metrics
          - '^process_.*' # Drop all process metrics
          - 'MY_UNWANTED_METRIC' # Drop a specific metric
```
