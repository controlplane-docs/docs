---
title: Capacity AI
description: Automatically optimize container CPU and memory allocation using historical usage analysis to reduce costs while maintaining performance.
keywords: ['right sizing', 'vpa', 'cost optimization', 'request tuning', 'auto adjust resources', 'min max cpu', 'min max memory', 'ml resource sizing']
---
...
## Availability by workload type

Capacity AI is supported on every workload type: standard, serverless, cron, stateful, and VM.

Standard, serverless, and cron workloads enable it by default.
...
On a stateful workload these fields stay bounded whether Capacity AI is enabled or disabled. See [Cost Optimization](/reference/workload/types#cost-optimization) for the limits.
...
When Capacity AI adjusts the CPU or memory allocations for containers, the corresponding Kubernetes pods will have their resources resized **in place**, without requiring a pod restart or recreation. This applies to standard and stateful workloads where the underlying cluster supports pod resizing. Where it does not, the new reservation is applied with a rolling update, as described in [Termination](/reference/workload/termination#when-termination-occurs).

Serverless workloads roll a new revision instead. Cron workloads apply the new reservation on their next scheduled execution, because every run starts a new pod. A run already in progress is never resized.
...
| Not available with GPU containers | GPU allocation is fixed, not dynamic |
