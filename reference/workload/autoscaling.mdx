---
title: Autoscaling
description: "Autoscaling strategies: disabled, concurrency-based, requests-per-second, CPU utilization, latency percentile, and memory-based with target values."
keywords: ['hpa', 'auto scale', 'replica scaling', 'min replicas', 'max replicas', 'rps scaling', 'p95', 'cold start', 'scale up', 'scale down', 'scaling policy', 'metric trigger']
---
...
The scale to zero functionality is available for Serverless workloads when using the `rps` or `concurrency` scaling strategies, and for Standard and Stateful workloads when using the `keda` metric. Cron workloads cannot scale to zero.

</Warning>

## Autoscaling Target

The target is the value that the system will try to keep the chosen scaling metric (such as `concurrency`, `rps`, `cpu`, `latency`, or `memory`) near but below.
...
Keda creates a Kubernetes HorizontalPodAutoscaler for the workload. Use `advanced.horizontalPodAutoscalerConfig.behavior`
to tune how that HPA scales in each direction, for example to slow down scale-down and avoid flapping, cap scale-up bursts,
or disable scaling in one direction. The block maps directly to the Kubernetes HPA
[`behavior`](https://kubernetes.io/docs/tasks/run-application/horizontal-pod-autoscale/#configurable-scaling-behavior) field.

`scaleUp` and `scaleDown` are both optional and accept the same fields:

- `stabilizationWindowSeconds`: Number of seconds past recommendations are considered before scaling. `0` to `3600`.
- `selectPolicy`: Which policy wins when several apply. `Max` (default), `Min`, or `Disabled` to turn off scaling in this direction.
- `policies`: One or more scaling policies. Each has a `type` of `Pods` or `Percent`, a `value` greater than `0`, and a `periodSeconds` between `1` and `1800`.
...
<Note>The name of the HPA is managed by Control Plane. The keda `horizontalPodAutoscalerConfig.name` field is not configurable.</Note>

### TriggerAuthentication

If your keda trigger requires authentication, store the credentials in a [dictionary secret](/reference/secret#dictionary) and add it to the [gvc keda configuration](/reference/gvc#keda).

For every secret listed under `spec.keda.secrets`, Control Plane creates a KEDA [TriggerAuthentication](https://keda.sh/docs/latest/concepts/authentication/#re-use-credentials-and-delegate-auth-with-triggerauthentication) object named after the secret.
Each key of the dictionary becomes an authentication parameter with the same name, so the keys must exactly match the parameter names the scaler expects.
Consult the [KEDA scaler documentation](https://keda.sh/docs/latest/scalers/) for the parameter names of your trigger type.

<Note>
Only secrets of type `dictionary` can be used as TriggerAuthentication objects. Store values as plain text; Control Plane handles the encoding.
</Note>

For example, a dictionary secret named `kafka-creds` with the keys `username` and `password` added to the gvc:
...
becomes a TriggerAuthentication named `kafka-creds` that provides the `username` and `password` parameters. A workload trigger references it by the secret name:
...
#### Example: Prometheus with a bearer token

The [KEDA prometheus scaler](https://keda.sh/docs/latest/scalers/prometheus/) expects a parameter named `bearerToken` when using bearer authentication.

1. Create a dictionary secret with a `bearerToken` key holding the token. If your Prometheus server uses a private CA, add its PEM under a `ca` key:
...
2. Add the secret to the gvc keda configuration:
...
3. Reference it from the workload trigger and set `authModes: bearer`:
...
<Tip>
Instead of inlining the token with `customHeaders`, store it in a dictionary secret under a `bearerToken` key and reference it via a [TriggerAuthentication](#triggerauthentication).
</Tip>
