...
- Maximum Concurrency (`autoscaling.maxConcurrency`)
  - The maximum allowed number of requests running against a single replica. If there are no replicas available processing less than the configured maximum number of concurrent requests, the system will queue requests and wait for a replica to be available. It will not trigger a scale decision. The purpose of this setting is to prevent a single replica from taking more traffic than it is designed to process.
  ...
  - When set to 0 the max is unlimited.
