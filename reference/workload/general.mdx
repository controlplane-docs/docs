...
Stopping a replica deletes it, and Control Plane starts a replacement to keep the replica count. Use it to clear one wedged replica without restarting the workload's other replicas. A `stateful` replacement keeps the same replica number and reattaches the same volume.
...
The workload must have been deployed to the location within the last 24 hours, or the command is rejected with a `404`. A second `stopReplica` for the same replica is rejected while the first is still in progress. Stopping a replica that a [`runCronWorkload`](/reference/workload/types#run-now) command started also cancels that command.
