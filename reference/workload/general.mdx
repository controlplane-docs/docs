---
title: General
description: "Workload reference overview covering access reports, identity assignment, the connect feature for debugging, and replica management across locations."
keywords: ['workload spec', 'cpln workload', 'cpln workload connect', 'exec into pod', 'replica scaling', 'identity binding', 'workload kind', 'spec fields', 'replica hostname', 'find replica hostnames', '/etc/hosts', 'replica dns', 'replica-direct endpoint']
---
...
Stopping a replica deletes it. For `standard`, `serverless`, and `stateful` workloads, Control Plane starts a replacement to keep the replica count, so stopping one is how you clear a wedged replica without restarting the others. A `stateful` replacement keeps the same replica number and reattaches the same volume. For a `cron` workload, stopping a replica deletes that run's job, and no replacement starts.

Stop a replica with the CLI using [`cpln workload replica stop`](/cli-reference/commands/workload#workload-replica-stop). `cpln workload replica get` lists the names of the running replicas.

```shell CLI Example
cpln workload replica stop my-workload --replica-name REPLICA_NAME --location aws-us-west-2 --gvc my-gvc --org my-org
```

The CLI submits a `stopReplica` [command](/concepts/command). You can also submit one yourself with `POST https://api.cpln.io/org/my-org/gvc/my-gvc/workload/my-workload/-command`:

```yaml stopReplica Command
type: stopReplica
spec:
...
- `location`
  - **Required**: The name of the location the replica runs in, as specified in your [GVC](/reference/gvc) configuration.
- `replica`
  - **Required**: The name of the replica to stop.

The command completes once the replica is gone. It fails if no replica with that name exists in the location. It can't be cancelled.

The workload must have a current [deployment](/reference/deployment) in the location, or the command is rejected with `404` `Workload: NAME does not have any deployments in LOCATION`. A deployment record that hasn't been updated for 24 hours counts as gone; see [stale and expired deployments](/reference/deployment#stale-and-expired-deployments). A second `stopReplica` for the same replica is rejected with `400` while the first is still in progress. Stopping a replica that a [`runCronWorkload`](/reference/workload/types#run-now) command started also cancels that command, unless it has already completed or failed.

Creating a `stopReplica` command requires the `exec.stopReplica` [permission](/reference/workload/security#permissions).
...
### Finding Replica Hostnames

When `replicaDirect` is enabled on a stateful workload, each replica is reachable at its own stable hostname, letting you target a specific replica instead of load-balancing across all of them:

- Internal (workload-to-workload): use the [replica-direct internal endpoint](#internal-endpoint-formatting), `replica-$replicaIndex.$workloadName.$locationName.$gvcName.cpln.local:$port`.
- External: use the [replica-direct endpoint](#replica-direct-endpoints), `$workloadName-$gvcAlias-$replicaIndex.$locationName.controlplane.us`.

Replica indexes start at `0` and run to one less than the replica count, so you can enumerate every replica's hostname from the workload name, location, GVC, and index. These names are served by Control Plane's internal DNS, so you construct them directly — there's no need to connect into the container to discover them.
...
[`spec.defaultOptions`](#suspend) applies to the workload in every [location](/reference/location) its [GVC](/reference/gvc) spans. `spec.localOptions` sets different options in named locations, and it is the only mechanism that does so.

It is an array. Each entry carries the same fields as `defaultOptions` plus a `location` link, which is required:
...
spec:
  defaultOptions:
    autoscaling:
      ...
      maxScale: 3
  localOptions:
    - location: //location/aws-eu-central-1
      autoscaling:
        minScale: 3
        maxScale: 10
...
The workload above runs 1 to 3 replicas everywhere its GVC reaches, 3 to 10 in `aws-eu-central-1`, and is switched off in `aws-us-west-2`.

Setting `suspend` in a `localOptions` entry stops the workload in that location without removing the location from the GVC. A workload with some locations suspended and others running reports `partiallySuspended`; see [Suspend](#suspend).
