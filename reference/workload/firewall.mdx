---
title: Firewall
description: "External and internal firewall rules for inbound and outbound traffic. Covers CIDR allowlists, hostname filtering, and HTTP header-based access control."
keywords: ['network policy', 'egress rules', 'ingress rules', 'allowlist', 'deny by default', 'security group', 'workload firewall', 'host filter', 'origin block', 'east-west traffic']
---
...
- `same-gvc`: Workloads running in the same [GVC](/reference/gvc) are accessible. Adding `inboundAllowWorkload` alongside it also allows the workloads named there, which may live in other GVCs. The Console shows that combination as `Same GVC & Workload List`, but it is stored as `same-gvc` with a populated list, not as a separate value.
