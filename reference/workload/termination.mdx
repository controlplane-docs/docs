...
If no custom preStop hook is defined for workload containers, Control Plane applies a default preStop hook that pauses the shutdown process by running `sh -c "sleep <duration>"`. 

The sleep duration is half of `terminationGracePeriodSeconds`; with the default 90-second grace period, the hook runs `sh -c "sleep 45"`. 
...
If the `sleep` executable is missing from one or more workload containers, all containers for the replica being shut down receive a `SIGKILL` signal immediately. 

Requests may still attempt to reach the containers and fail before the load balancer is fully updated.
