...
A **cloud account** is a mapping between your [org](org) and a cloud provider. When creating a cloud account, instructions are provided on how to create a bridge account at the cloud provider with minimum access permissions. This bridge account is used by Control Plane to query and manage the necessary resources at the cloud provider on behalf of your [org](org).

Cloud accounts are scoped to an [org](org) and are used in conjunction with [identities](identity) to set up [cloud access](identity#cloud-access-universal-cloud-identity) rules.

Your [workload](workload) can then be associated with an [identity](identity) and will then have the ability to call any of the allowed cloud provider resources transparently without any additional set up.

For example, if your application uses AWS S3 for storage and Azure Cosmos DB for its database, your workload will be able to access both no matter which [location](location) the workload is deployed to. The [workload](workload) calls each resource as if it had a direct connection and the [identity](identity) will route the request seamlessly to the proper location.

## Create a Cloud Account

Refer to the [Create a Cloud Account](/guides/create-resources/create-cloud-account) guide for additional details.
...
- Identity Creation
  ...
  - When an [identity](identity) is created targeting AWS, Control Plane will create a new role in AWS that will have the minimum permissions that are required to access the targeted services. When a [workload](workload) that is assigned with an [identity](identity) requests credentials to access AWS services, Control Plane will obtain a temporary access token that impersonates the role and inject it into the [workload](workload) granting it access.

- Removing an AWS Cloud Account
  - If you no longer require Control Plane to create identities targeting AWS:
    - Delete all cloud access rules targeting AWS from existing workload [identities](identity). This will delete any associated roles.
...
During the creation of an Azure cloud account, the Azure CLI generates credentials that are used by Control Plane when connecting to Azure to provision apps or user identities that will be used by workload identities. These credentials are uploaded and stored securely as an [Azure-SDK secret](secret#azure-sdk) or [Azure-Connector secret](secret#azure-connector) and can be viewed by clicking `Secrets` from the left menu in the console after creating an Azure cloud account.
...
- Cloud Account Creation
  ...
  - The name of this service principal is the name of your [org](org) prefixed with `cpln-` (i.e., cpln-ORG_NAME).
  ...
  - By granting this permission, the cloud account will be able to create/manage App registrations and query available Azure scopes and roles during the creation of an [identity](identity).
...
For every [identity](identity) created, Control Plane creates an App registration for which it generates short-lived credentials and injects them into your [workload](workload) using the native cloud providers identity interface. No matter which cloud your workload is running, Control Plane ensures that the identity information is conveyed correctly to the consumed services. Control Plane mints tokens for the [identity](identity) bound to the workloads.

</Info>

- Identity Creation

  - When creating an [identity](identity), [cloud access](identity#cloud-access-universal-cloud-identity) rules targeting Azure can be defined which specify the minimum access needed by the identity. When the [identity](identity) is saved in the console, the Azure cloud account will leverage the service principal to create a new App registration with the configured access.
  - Any [workload](workload) can be configured to use this [identity](identity) and will have access to the defined resources.
...
- Expired / Compromised Credentials
  ...
  - Until a valid secret value has been updated, Control Plane will not be able to manage workload [identities](identity).

- Removing an Azure Cloud Account

  - If you no longer require Control Plane to create identities targeting Azure:

    - Delete all cloud access rules targeting Azure from existing workload [identities](identity). This will delete any associated app registrations.
    - Delete the cloud account.
    - Delete the [azure secret](secret#azure-sdk) that was mapped to the cloud account (named: CLOUD_ACCOUNT-access).
...
- Cloud Account Creation
  ...
  - After the registration is completed, a secret of type [azure-connector](secret#azure-connector) will be created. This secret can be reused to create another cloud account (using the same Function App) or if an existing cloud account was inadvertently deleted.
...
- Identity Creation
  ...
  - When an [identity](identity) is created targeting Azure resources, a managed identity is assigned to the function so that the function can act on its behalf. This [identity](identity) will only have the minimum permissions that are required to access the targeted services. When an [identity](identity) is assigned to a [workload](workload) that requests access to an Azure resource, Control Plane will obtain a temporary access token that impersonates the roles and inject it into the [workload](workload) granting it access.
...
- Compromised Code
  ...
  - Until a valid code has been updated, Control Plane will not be able to manage workload [identities](identity).

- Removing an Azure Cloud Account

  - If you no longer require Control Plane to create identities targeting Azure:

    - Delete all cloud access rules targeting Azure from existing workload [identities](identity). This will delete any user assigned identities.
    - Delete the cloud account.
...
- Identity Creation
  ...
  - When an [identity](identity) is created targeting GCP, Control Plane will create a new `Service Account` in your GCP account that will have the minimum permissions that are required to access the targeted services.
  - When a [workload](workload) that is assigned with an [identity](identity) requests credentials to access GCP services, Control Plane will obtain a temporary access token that impersonates the service account and inject it into the [workload](workload) granting it access.
  - If the [identity](identity) created connects with a particular GCP service, the Control Plane GCP service account will need to be granted the `Admin` role for that service.
    - For example, if the [identity](identity) will be using Cloud Store, the `cpln-ORG_NAME@ENV.iam.gserviceaccount.com` service account needs to have the `Storage Admin` role granted to it. By adding this role, Control Plane will be able to grant temporary access token to the workload when reading/writing to a storage bucket.
...
If your workload [identities](identity) will be using a lot of GCP services, instead of granting the `Admin` role for each service, it will be easier to grant the `cpln-ORG_NAME@ENV.iam.gserviceaccount.com` service account the `Owner` role.

</Tip>

- Removing a GCP Cloud Account
  - If you no longer require Control Plane to create identities targeting GCP:
    - Delete all cloud access rules targeting GCP from existing workload [identities](identity). This will delete any associated service account.
    - Delete the cloud account.
