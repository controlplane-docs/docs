...
For example, if an application uses AWS S3 for storage and Azure Cosmos DB for its database, a workload can access both services regardless of the [location](/reference/location) in which the workload is deployed to. The [workload](/concepts/workload)interacts with each service as if it were directly connected, while the [identity](/reference/identity) handles secure routing and credential exchange transparently.

## Create a Cloud Account

Refer to the [Create a Cloud Account](/guides/create-cloud-account) guide for setup instructions.
...
- Identity Creation
  ...
  - When an [identity](/reference/identity) is targets AWS, Control Plane creates an IAM role with least-privilege permissions for the requested services.
