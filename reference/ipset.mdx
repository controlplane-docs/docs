---
title: IP Set
description: Reserve and manage public IP addresses per GVC location for workloads, including binding, permissions, and configuration options.
keywords: ['static ip', 'reserved ip', 'egress ip', 'ipv4', 'allowlist', 'firewall source', 'fixed ip', 'cpln ipset', 'elastic ip']
---

## Overview

An IP Set reserves a public IP address for each location configured within a Global Virtual Cloud
([GVC](/reference/gvc)). The public IP address is stored in the object’s status. When a new location is added to a
[GVC](/reference/gvc), a new IP address is automatically created if one does not already exist for that location.
To prevent ongoing charges, IP addresses must be explicitly released once they are no longer needed.

## Binding

An IP Set can be linked to a [workload](/reference/workload) or a [gvc](/reference/gvc) through its `spec.link` property.

When linking to a [workload](/reference/workload), the workload must have [direct load balancer](/reference/workload/load-balancing#direct) enabled. A [workload](/reference/workload) MUST link to the IP Set via its [direct load balancer](/reference/workload/load-balancing#direct).
