...
description: Integrate your own Kubernetes cluster into Control Plane as a new location, enabling managed workload deployment on your own infrastructure.
keywords: ['bring your own kubernetes', 'self-hosted', 'on-premise', 'on-prem', 'private cloud', 'eks', 'gke', 'aks', 'k3s', 'rke', 'data center', 'self-managed cluster', 'k8s', 'cpln.io/nodeType']
---
...
This is how workloads run on hardware you own — bare-metal servers in a colo, on-premises virtual machines, or a private cloud — rather than in a public cloud region. If you have servers but no Kubernetes cluster yet, the [Managed Kubernetes Generic provider](/mk8s/generic) builds one from them first. For the end-to-end walkthrough, see [Deploy a Workload to Your Own Hardware](/guides/deploy-to-your-own-hardware).
...
<Frame>
  <img src="/images/byok/install-location.png" alt="Control Plane console showing the Actions dropdown with Install option for a BYOK location" />
</Frame>

2. Copy the generated kubectl command. Apply it within about 5 minutes — the command fetches the manifests, which contain sensitive tokens, from a signed URL that expires. If it expires, generate a fresh command.
...
<Frame>
  <img src="/images/byok/install-location.png" alt="Control Plane console showing the Actions dropdown with Uninstall option for a BYOK location" />
</Frame>
...
4. A job with the name prefix `cpln-agent-uninstall-` will be created in the `kube-system` namespace. It will clean up all BYOK components from the cluster.
