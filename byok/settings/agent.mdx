...
Installs the JuiceFS CSI driver and a Redis Sentinel metadata engine in the `juicefs` namespace. Shared file system volume sets need it, along with an object storage configuration; see [Shared File System](/byok/settings/volumeset#shared-file-system).
