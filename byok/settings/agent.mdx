...
description: Configure BYOK agent settings via the Control Plane API or the cpln-byok-current configmap for self-managed Kubernetes clusters.
keywords: ['environment variables', 'kube-system', 'kubectl get configmap', 'cpln-byok-default', 'self-hosted', 'cluster settings', 'agent options', 'k8s']
---
...
Installs the JuiceFS CSI driver and a Redis metadata engine (`juicefs-redis`, monitored by a three-node Sentinel) in the `juicefs` namespace. The Redis uses three 4Gi PersistentVolumeClaims on the cluster's default StorageClass. Shared file system volume sets need it, along with an object storage configuration; see [Shared File System](/byok/settings/volumeset#shared-file-system).

| Property   | Type    | Description                                              |
| :--------- | :------ | :------------------------------------------------------- |
| enabled    | boolean | `true` installs JuiceFS. Defaults to `false`.            |
